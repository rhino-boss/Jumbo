...
if True:
    ...
    def generate_obj_area(layout_status, size, div, div_type):
        ...
        def get_element_posi(div_list, total_len):
            num = len(div_list)
            element_uw = (total_len - (num - 1) * GS_sw) / sum(
                div_list
            )  # element 寬的單位
        ...
        for r in element_row_list:
            for c in element_col_list:
                if div_type == "base":
                    emt = ObjElement(
                        c[0], r[0] + layout_status.get_cum_y(), c[1], r[1], div_type
                    )
                elif div_type == "no_sw":
                    emt = ObjElement(
                        c[0], layout_status.get_cum_y() + GS_sw, c[1], r[1], div_type
                    )
    ...
    f = lambda h, div, div_type: generate_obj_area(
        layout_status, (layout_status.layout_w - 20, h), div, div_type
    )
    ...
    symbol_area = f(
        symbol_h * MM.window_size + GS_sw * (MM.window_size - 1), [1], "base"
    )[0]
    spinscore_area, autospin_area, spin_area = f(GS_fsh, [2, 1, 1], "base")

    (
        trigger_freegame_area,
        info1_area,
        info2_area,
        info3_area,
        info4_area,
        info5_area,
    ) = f(GS_fsh * 6 + GS_sw * 5, [1, 1, 1, 1, 1, 1], "no_sw")
...
if True:
    ...
    class Symbol(pg.sprite.Sprite):
        ...
        def __init__(
            ...
            sy,
            simg,
            limit=GS.symbol_move_limit,
            shadow_value=GS.alpha_max,
        ):
            # intial setting
    ...
    class Button(pg.sprite.Sprite):
        ...
        def __init__(
            ...
            position,
            size,
            background_color=GS.Color.gray,
            text_color=GS.Color.white,
            is_info=False,
            click_text="",
        ):
        ...
        def _font_set(self, text):
            ...
            if self.is_info:
                ...
                self.text_center = self.text.get_rect(
        ...
        def click_one(self, gs):
            if gs.math_sts.game_type == 1:
                return
    ...
    class GameStatus:
        def __init__(self) -> None:
            # [畫面]
            # - layout setting
            self.threshold_reel_roll_delay = [
                i * GS.reel_wait for i in range(MM.reel_num)
            ]  # reel 間滾動延遲
    ...
    def generate_objects(show_arr, show_type, show_posi=zeros(MM.arr_shape)):
        # output
        reel_groups = []
        symbol_objs = []
        ...
        for j in range(arr_shape[1]):  # 5 (reel num)
            ...
            for i in range(arr_shape[0]):  # 3 (window size)
                ...
                if show_type == "base":
                    ...
                    shadow_value = (
                        GS.alpha_payline if show_posi[i, j] == 0 else GS.alpha_max
                    )
                    s = Symbol(
                        GS.move_unit, _sw, _sh, img, shadow_value=shadow_value, limit=0
                    )
    ...
    def one_spin(gs, is_auto=False):
        ...
        # [one spin] 數學資料
        gs.math_sts.one_spin()

        # [append] append "show_datas"
        # - 設定sticky在reel上的物件
        show_reels, symbol_objs = generate_objects(
            gs.math_sts.arr_sticky_symbol,
            "sticky",
            show_posi=gs.math_sts.arr_sticky_posi,
        )  # 建立全部物件群組
        stk = ShowData(
            show_reels, symbol_objs, "sticky", gs.math_sts.math_res.show_score
        )
        gs.show_datas.append(stk)

        # - 設定reel上的物件
        show_reels, symbol_objs = generate_objects(
            gs.math_sts.math_res.arr_result, "base"
        )
        ori = ShowData(show_reels, symbol_objs, "base", gs.math_sts.math_res.show_score)
        ori_last = ShowData(
            show_reels, symbol_objs, "last", gs.math_sts.math_res.show_score
        )
        ...
        for pl, txt in gs.math_sts.math_res.pay_lines:
            show_reels, symbol_objs = generate_objects(
                gs.math_sts.math_res.arr_result, "payline", show_posi=pl
            )
            gs.show_datas.append(
                ShowData(
                    show_reels,
                    symbol_objs,
                    "payline",
                    gs.math_sts.math_res.show_score,
                    payline_text=txt,
                )
            )
        ...
        # [畫面]
        if gs.math_sts.game_type == 0:
            print("aa")
            trigger_freegame_btn.show_text = "aa"
...
if True:
    ...
    score_btn = Button(
        "score: 0", spinscore_area.posi, spinscore_area.size, GS.Color.gray
    )
    btns.add(score_btn)
    ...
    trigger_freegame_btn = Button(
        "free game",
        trigger_freegame_area.posi,
        trigger_freegame_area.size,
        GS.Color.gray,
        text_color=GS.Color.white,
        click_text="click",
    )
    ...
    # [物件設定] (初始盤面)
    show_reels, symbol_objs = generate_objects(
        zeros(MM.arr_shape), "payline"
    )  # 建立初始物件群組(假的盤面)
    gs.show_datas = [
        ShowData(show_reels, symbol_objs, "payline", 0, payline_text="game start")
    ]
...
if True:
    ...
    def game_info_setting(game_setting):
        ...
        _show_hit_rate = (
            gs.math_sts.cnt_hits / gs.math_sts.cnt_spins
            if gs.math_sts.cnt_spins > 0
            else 0
        )
        _show_trigger_rate = (
            int(gs.math_sts.cnt_spins / gs.math_sts.cnt_trigger_freegame_times)
            if gs.math_sts.cnt_trigger_freegame_times > 0
            else "inf"
        )
        ...
        if gs.math_sts.game_type == 0:
            ...
            round_btn.update_btn(
                f"free game ({gs.math_sts.cnt_free_spins}/{gs.math_sts.have_free_spins})",
                color=GS.Color.red,
            )
        ...
        info1_btn.update_btn(
            f"coin: {gs.math_sts.credit} ( +{gs.math_sts.freegame_coin} )"
        )
        info2_btn.update_btn(
            f"hit rate: {_show_hit_rate:0.2f}  ( hits: {gs.math_sts.cnt_hits} / spins: {gs.math_sts.cnt_spins} )"
        )
        info3_btn.update_btn(
            f"max score: {gs.math_sts.max_score_basegame} / {gs.math_sts.max_score_freegame}"
        )
        info4_btn.update_btn(
            f"free game trigger: {gs.math_sts.cnt_trigger_freegame_times} ( period: {_show_trigger_rate} )"
        )
    ...
    while gs.flag:
        ...
        if gs.show_now.type in ("payline", "last"):
            gs.threshold_reel_roll_delay = [
                0 for i in range(MM.reel_num)
            ]  # reel 間滾動延遲
        else:
            gs.threshold_reel_roll_delay = [
                i * GS.reel_wait for i in range(MM.reel_num)
        ...
        for event in pg.event.get():
            ...
            if (
                (is_spin_click or is_space_click)
                and lock == False
                and autospin_btn.on == False
            ):
                lock = init_cnt(gs)
                one_spin(gs)

            # auto spin
            is_auto_spin_click = mouse_type and autospin_btn.rect.collidepoint(
                event.pos
            )
            ...
            # trigger free game
            is_trigger_freegame_click = (
                mouse_type and trigger_freegame_btn.rect.collidepoint(event.pos)
            )
            if (
                is_trigger_freegame_click
                and lock == False
                and gs.math_sts.game_type == 0
            ):
                lock = init_cnt(gs)
                trigger_freegame_btn.click_one(gs)
