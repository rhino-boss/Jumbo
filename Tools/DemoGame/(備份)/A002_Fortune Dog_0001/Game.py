...
if True:
    ...
    class GameStatus:
        def __init__(self) -> None:
            ...
            # - math (暫存)
            self.math_res = MM.MathOutput()  # 每呼叫一次"Math"刷新一次
        ...
        def init_sticky(self):
            self.math_sts.arr_sticky_symbol = self.math_res.arr_sticky_symbol_init
            self.math_sts.arr_sticky_posi = self.math_res.arr_sticky_posi_init

        def update_sticky(self):
            self.math_sts.arr_sticky_symbol = self.math_res.arr_sticky_symbol
            self.math_sts.arr_sticky_posi = self.math_res.arr_sticky_posi
    ...
    def one_spin(gs, is_auto=False):
        ...
        # [判斷遊戲模式] base game/free game/...
        if (gs.game_type == 0 and gs.math_res.trigger_freespins > 0) or (gs.math_sts.have_free_spins > gs.math_sts.cnt_free_spins):  # 在BG有freespins或在FG還有freespins
            gs.game_type = 1
        else:
            gs.game_type = 0

            # clear free game data
            gs.math_sts.credit += gs.math_sts.freegame_coin
            gs.math_sts.freegame_coin = 0
            gs.math_sts.have_free_spins = 0
            gs.math_sts.cnt_free_spins = 0

        # [跟math要資料]
        if gs.game_type == 0:
            gs.math_res = MM.basegame_spin_and_calculate()
            gs.init_sticky()
        elif gs.game_type == 1:
            gs.math_res = MM.freegame_spin_and_calculate()
            gs.update_sticky()

        # [show_datas] append "show_datas"
        # - 設定sticky在reel上的物件
        ...
        stk = ShowData(show_reels, symbol_objs, "sticky", gs.math_res.show_score)
        gs.show_datas.append(stk)

        # - 設定reel上的物件
        show_reels, symbol_objs = generate_objects(gs.math_res.arr_result, "base")
        ori = ShowData(show_reels, symbol_objs, "base", gs.math_res.show_score)
        ori_last = ShowData(show_reels, symbol_objs, "last", gs.math_res.show_score)
        gs.show_datas.append(ori)

        # - 設定payline
        for pl, txt in gs.math_res.pay_lines:
            show_reels, symbol_objs = generate_objects(gs.math_res.arr_result, "payline", show_posi=pl)
            gs.show_datas.append(ShowData(show_reels, symbol_objs, "payline", gs.math_res.show_score, payline_text=txt))
        if len(gs.math_res.pay_lines) > 0 or is_auto:
            gs.show_datas.append(ori_last)  # show use

        # [show_now] pop "show_datas"
        gs.cnt_reel_wait = 0
        ...
        # - 改變顯示的分數
        if gs.game_type == 0:
            gs.math_sts.credit -= MM.coin_in
            gs.math_sts.credit += gs.math_res.show_score
            if gs.math_res.show_score > 0:
                gs.math_sts.cnt_hits += 1
            if gs.math_res.show_score > gs.math_sts.max_score_basegame:
                gs.math_res.max_score_basegame = gs.math_res.show_score
            gs.math_sts.cnt_spins += 1
        elif gs.game_type == 1:
            gs.math_sts.freegame_coin += gs.math_res.show_score
            if gs.math_res.show_score > gs.math_sts.max_score_freegame:
                gs.math_res.max_score_freegame = gs.math_res.show_score
            gs.math_sts.cnt_free_spins += 1

        # - 計算free spin (有影響到資料，待調整)
        if gs.math_res.trigger_freespins > 0:
            gs.math_sts.cnt_trigger_freegame_times += 1
            gs.math_sts.have_free_spins += gs.math_res.trigger_freespins
            if gs.math_sts.have_free_spins >= MM.max_freespins:
                gs.math_sts.have_free_spins = MM.max_freespins
...
if True:
    ...
    trigger_freegame_btn = Button("free game", trigger_freegame_area.posi, trigger_freegame_area.size, GS.Color.gray, text_color=GS.Color.white)
...
if True:
    ...
    def game_info_setting(game_setting):
        ...
        info5_btn.update_btn(f"rng: {gs.math_res.rng} {gs.game_type}")
    ...
    while gs.flag:
        ...
        for event in pg.event.get():
            ...
            if is_trigger_freegame_click and lock == False and gs.game_type == 0:
                print("* trigger freegame")
                lock = init_cnt(gs)
