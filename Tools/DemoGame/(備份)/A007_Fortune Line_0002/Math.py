...
if True:
    ...
    rng_freegame = np.array([3, 3, 10, 0, 0], np.int16)
...
def basegame_spin_and_calculate(show_info="", rng_freegame=np.zeros(0)):
    ...
    def arr_result_generator(game_type):
        ...
        if game_type == 0:  # base
            ...
            for i in range(window_size):
                arr_result[i, 0] = F6R1[(rng[0] + i) % F6R1_len]
            ...
            for i in range(window_size):
                arr_result[i, 0] = F7R1[(rng[0] + i) % F7R1_len]
    ...
    def one_spin_feature_bg(trigger_symbol):
        ...
        print_result("[{0}]base game - feature".format(table + 1), arr_result, rng, score, arr_result_uup)
        return score, table
...
def freegame_spin_and_calculate(show_info=""):
    ...
    def one_spin_feature_fg(feature_data):
        ...
        if show_log:
            print(arr_result)
            print_result(f"[{reel_id}]free game - feature", arr_result, rng, score, arr_result_uup)
