...
if True:
    ...
    class GameStatus:
        def __init__(self) -> None:
            # [畫面]
            # - layout setting
            self.threshold_reel_roll_delay = [i * GS.wait_reel for i in range(MM.reel_num)]  # reel 間滾動延遲
...
if True:
    ...
    while gs.flag:
        ...
        lock = gs.game_cnt <= GS.wait_update
        ...
        if gs.show_now.type in ("payline", "last"):
            ...
            gs.threshold_reel_roll_delay = [i * GS.wait_reel for i in range(MM.reel_num)]  # reel 間滾動延遲
        ...
        # -- auto spin (自動)
        if autospin_btn.on and len(gs.show_datas) == 0 and lock == False and not gs.no_credit:
            one_spin(gs, True)
        ...
        for event in pg.event.get():
            ...
            gs.no_credit = gs.math_sts.credit <= MM.coin_in
            ...
            if (is_spin_click or is_space_click) and lock == False and autospin_btn.on == False and not gs.no_credit:
                lock = init_cnt(gs)
                spin_btn.click()
                if spin_btn.on:
                    one_spin(gs)
            ...
            if gs.no_credit and not gs.no_credit_click and autospin_btn.on:
                gs.no_credit_click = True
                autospin_btn.click()

            # auto spin
            is_auto_spin_click = mouse_type and autospin_btn.rect.collidepoint(event.pos)
            is_a_click = keys[pg.K_a]
            if (is_auto_spin_click or is_a_click) and lock == False and not gs.no_credit:
                lock = init_cnt(gs)
                autospin_btn.click()

            # trigger free game
            is_trigger_freegame_click = mouse_type and trigger_freegame_btn.rect.collidepoint(event.pos)
            if is_trigger_freegame_click and lock == False and gs.math_sts.game_type == 0:
                lock = init_cnt(gs)
