...
# rank_0_20_area, rank_20_40_area, rank_40_60_area, rank_60_80_area, rank_80_100_area = f(GS_fsh, [1, 1, 1, 1, 1], "base")

info1_area, info2_area, info3_area, info4_area, info5_area, info6_area = f(GS_fsh * 6 + GS_sw * 5, [1, 1, 1, 1, 1, 1], "no_sw")
...
class GameStatus:
    def __init__(self) -> None:
        ...
        self.spins = 0  # base game累積轉數
        self.hits = 0  # base game有得分次數(不包含免費遊戲)

        self.base_max_score_one = 0  # 單把最大分數(base game)
        self.free_max_score_one = 0  # 單把最大分數(free game)

        self.trigger_freegame_times = 0  #  免費遊戲進行次數

        self.freegame_score_stack = []  # 目前有幾個免費遊戲
        self.free_score = 0  # 免費遊戲C1分數
        self.free_spins = 0  # 有幾次free spin
        self.free_spins_cnt = 0  # 該場免費遊戲進行了幾次spin
        ...
        self.rng = "--"
        self.paylines = []
        self.trigger_score_stack = []

        # Data
        self.show_datas = []
        self.show_now = None
        self.game_type = 0  # 0: base game, 1: free game
...
def one_spin(game_setting, is_auto=False):
    gs = game_setting
    ...
    # spin
    is_last_freespin = gs.free_spins == gs.free_spins_cnt and gs.game_type == 1
    have_freegame = len(gs.freegame_score_stack) > 0
    if (gs.game_type == 0 or is_last_freespin) and have_freegame:
        gs.game_type = 1
        gs.free_score = gs.freegame_score_stack.pop(0)
        gs.free_spins = MM.random_spins()
        gs.free_spins_cnt = 0
        gs.trigger_freegame_times += 1
    elif gs.free_spins > gs.free_spins_cnt:
        gs.game_type = 1
    else:
        gs.game_type = 0
        gs.coin += gs.freegame_coin
        gs.freegame_coin = 0

    if gs.game_type == 0:
        math_res = MM.basegame_spin_and_calculate()
    else:
        math_res = MM.freegame_spin_and_calculate(gs.free_score)
    ...
    gs.trigger_score_stack = math_res.trigger_score_stack

    if len(gs.trigger_score_stack) > 0:
        gs.freegame_score_stack.extend(gs.trigger_score_stack)

    # + score
    if gs.game_type == 0:
        gs.coin -= MM.coin_in
        gs.coin += gs.show_score
    else:
        gs.freegame_coin += gs.show_score
    ...
    ori_last = dict(reel=show_reels, symbols=symbol_objs, type="last", score=gs.show_score, payline_text="")
    ...
    if len(gs.paylines) > 0 or is_auto:
    ...
    if gs.game_type == 0:
        ...
        if gs.show_score > gs.base_max_score_one:
            gs.base_max_score_one = gs.show_score

        gs.spins += 1
        gs.spin_cum = 1

    elif gs.game_type == 1:
        if gs.show_score > gs.free_max_score_one:
            gs.free_max_score_one = gs.show_score
...
# rank_0_20_btn = Button("0-20%", rank_0_20_area.posi, rank_0_20_area.size, GS.Color.gray, click_text="inf")
# btns.add(rank_0_20_btn)

# rank_20_40_btn = Button("20-40%", rank_20_40_area.posi, rank_20_40_area.size, GS.Color.gray, click_text="inf")
# btns.add(rank_20_40_btn)

# rank_40_60_btn = Button("40-60%", rank_40_60_area.posi, rank_40_60_area.size, GS.Color.gray, click_text="inf")
# btns.add(rank_40_60_btn)

# rank_60_80_btn = Button("60-80%", rank_60_80_area.posi, rank_60_80_area.size, GS.Color.gray, click_text="inf")
# btns.add(rank_60_80_btn)

# rank_80_100_btn = Button("80-100%", rank_80_100_area.posi, rank_80_100_area.size, GS.Color.gray, click_text="inf")
# btns.add(rank_80_100_btn)
...
info6_btn = Button("--", info6_area.posi, info6_area.size, GS.Color.white, text_color=GS.Color.black, is_info=True)
btns.add(info6_btn)
...
def game_info_setting(game_setting):
    ...
    _show_trigger_rate = int(gs.spins / gs.trigger_freegame_times) if gs.trigger_freegame_times > 0 else "inf"

    if gs.game_type == 0:
        round_btn.update_btn(GS.format_bg_round_btn)
    else:
        round_btn.update_btn(f"free game ({gs.free_spins_cnt}/{gs.free_spins})({gs.free_score})", color=GS.Color.red)

    score_btn.update_btn(f"score: {gs.show_now['score']}")
    payline_btn.update_btn(gs.show_now["payline_text"])

    info1_btn.update_btn(f"coin: {gs.coin} ( +{gs.freegame_coin} )")
    info2_btn.update_btn(f"hit rate: {_show_hit_rate:0.2f}  ( hits: {gs.hits} / spins: {gs.spins} )")
    info3_btn.update_btn(f"max score: {gs.base_max_score_one} / {gs.free_max_score_one}")
    info4_btn.update_btn(f"free game trigger: {gs.trigger_freegame_times} ( period: {_show_trigger_rate} )")
    info5_btn.update_btn(f"free game stack: {gs.freegame_score_stack}")
    info6_btn.update_btn(f"rng: {gs.rng} {gs.game_type}")


def init_cnt(game_setting):
    game_setting.game_cnt = 0
    return gs.game_cnt <= 10
...
while gs.flag:
    ...
    # -- auto spin (自動)
    if autospin_btn.on and len(gs.show_datas) == 0 and lock == False:
        one_spin(gs, autospin_btn.on)
    ...
    for event in pg.event.get():
        ...
        if (is_spin_click or is_space_click) and lock == False and autospin_btn.on == False:
            lock = init_cnt(gs)
            one_spin(gs)
        ...
        if (is_auto_spin_click or is_a_click) and lock == False:
            lock = init_cnt(gs)
            autospin_btn.click()
