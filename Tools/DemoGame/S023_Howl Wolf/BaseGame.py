...
def one_spin(game_setting, is_auto=False):
    ...
    #
    gs.trigger_freegame_times += len(gs.trigger_score_stack)
...
def game_info_setting(game_setting):
    ...
    info1_btn.update_btn(f"hit rate: {_show_hit_rate:0.2f}  ( hits: {gs.hits} / spins: {gs.spins} )")
    info2_btn.update_btn(f"max score: {gs.base_max_score_one}")
    info3_btn.update_btn(f"free game trigger: {gs.trigger_freegame_times} ( period: {_show_trigger_rate} )")
    info4_btn.update_btn(f"rng: {gs.rng} {gs.game_type}")
