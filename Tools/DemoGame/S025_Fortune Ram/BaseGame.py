...
gs.show_datas = [dict(reel=show_reels, symbols=symbol_objs, type="payline", score=0, payline_text="game satrt")]
...
while gs.flag:
    ...
    _show_trigger_rate = int(gs.spins / gs.trigger_cum) if gs.trigger_cum > 0 else "inf"

    round_btn.update_text(GS.format_bg_round_btn)
    ...
    score_btn.update_text(f"score: {gs.show_now['score']}")
    info2_btn.update_text(f"max score: {gs.max_score_one}")
    info3_btn.update_text(f"free game trigger: {gs.trigger_cum} ( period: {_show_trigger_rate} )")
