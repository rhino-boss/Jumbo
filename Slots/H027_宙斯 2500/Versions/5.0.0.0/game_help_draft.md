...
| 規則說明 | ▪ 所有獎金仍依您所選的基礎投注計算。 | ▪ ALL WINS ARE STILL CALCULATED ON THE BASE BET YOU SELECT. |
...
| 規則說明 | ▪ OP JACKPOT 特色會在畫面中顯示 {12} 個 [幣]，其中包含 [幣-GRAND]、[幣-MAJOR]、[幣-MINOR] 與 [幣-MINI]。 | ▪ DURING OP JACKPOT FEATURE, THERE ARE {12} [COIN] WHICH CONSIST OF [COIN-GRAND], [COIN-MAJOR], [COIN-MINOR], AND [COIN-MINI]. |
| 規則說明 | ▪ 選擇 {1} 個 [幣]，以揭示 [幣-GRAND]、[幣-MAJOR]、[幣-MINOR] 或 [幣-MINI]。獲得 {3} 個相同的 OP JACKPOT 符號，即可贏得對應彩金並結算總獎金。 | ▪ SELECT {1} [COIN] TO REVEAL [COIN-GRAND], [COIN-MAJOR], [COIN-MINOR], OR [COIN-MINI]. OBTAIN {3} IDENTICAL OP JACKPOT SYMBOLS TO WIN THE CORRESPONDING JACKPOT AND CALCULATE THE TOTAL WIN. |
...
> **待確認**：僅剩 [GRAND]／[MAJOR] 的解鎖門檻金額。`{2.00}` 是 H019 的值，
> H027 的實際門檻需向 JP 設定端（`JP optionID 28`）確認後回填本區塊與 `game_rule.md` §6.4。
> 其餘規則已於 2026-09-22 寫入 `game_rule.md` §6.4（OP／OmniPlay 為跨遊戲共用彩金模組，規則同 H019）。
...
| 規則說明 | ▪ 一般符號獎金為「賠率 × 投注」。 | ▪ A REGULAR SYMBOL WIN IS CALCULATED AS THE PAYTABLE VALUE × BET. |
