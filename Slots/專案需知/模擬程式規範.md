...
<rtp_game_id>_<rtp_version_tag>_<timestamp>_betmode<bet_mode>_<rounds_tag>_<rtp_tag>_<profile>[_<bet_tier>].xlsx
```
...
> Card System On／Off 由檔名結構區分（On 帶 `rtp_tag`＋`profile` 段），檔名**不加** `card` 後綴。
...
H016192A_02000000_2608141341_betmode0_105_9600_oldhand_medium_bet.xlsx

Card System On（Feature Buy，Oldhand 小 Bet）：
H026194A_02010313_2608141530_betmode2_108_9650_oldhand_small_bet.xlsx
```
...
- **`base_game_cnt`／`base_game_pay` 與所有 `BG_*` 統計只計「未觸發 FG」的局**：倍率權重分「倍數卡（區間卡）」與「FG 卡」兩類，BG Multiplier Line 是倍數卡的推導來源，觸發 FG 的局屬 FG 卡口徑，不得混入；觸發局的 BG 得分一律進 `bg_trigger_fg_*_lte_upper` 累計欄。
- `Interval_Upper` 為該列倍率區間的數值上限；`bg_trigger_fg_cnt_lte_upper` 與 `bg_trigger_fg_pay_lte_upper` 分別累計所有「BG 成功觸發 FG，且該把 BG 倍數小於等於該列上限」的次數與 BG 得分。兩欄必須由未套 Profile cap 的原始觸發 BG 分桶累加，不得先過濾成單一 Profile。此兩欄是天花板機制（見數學模型規範 §2.3：觸發把超過卡組上限即重骰）的配套資料——`pay_lte_upper ÷ cnt_lte_upper` 於該 Profile 卡組上限那一列，即為 Detail 之 Free Game 列應填的截斷平均倍率。
...
`BATCH_RUNS` 依下列四個區塊組織；當下用不到的區塊以註解保留，不刪除：

| 區塊 | 用途 | Card System | 彩金 | NB／EB 場次 | BF／SF 場次 |
|---|---|---|---|---:|---:|
| 1. 測試 | 流程與報表驗證 | On | 依需求 | `10^5` | `10^5` |
| 2. 自然機率 | 自然機率基準 | Off | 關 | `10^9` | `10^8` |
| 3. SCR | 量測 SCR 供 OP Jackpot 頁使用 | On | 關 | `10^8` | `10^7` |
| 4. 正式模擬 | 確認正確性（含彩金與押注層級） | On | 開（依搭配表） | `10^8` | `10^7` |

- **SCR 區塊只跑必要組合**：權重完全相同的組合只跑一筆代表——例如 Newbie 權重各 RTP／Variant 共用時只跑一筆、Buy Feature 權重各版共用時只跑一筆；Oldhand NB 各版權重不同則各跑一筆。
- **自然機率區塊的組成依模型設計而定**：BF／SF 的倍率權重若引用 NB FG 分布（如 H028 的 BF Detail 共用 NB FG 自然分布），自然機率只需 `bet_mode 0`，不另跑 BF／SF；**此區塊要跑哪些 Bet Mode，製作前必須先與使用者確認**，不得自行假設。
- 正式模擬區塊沿用相同精簡原則；押注層級邊界（`$2`、`$100`、`>$100`）與 Feature 購買價層級必須各有代表批次。
- 各區塊內的組合仍須符合第 3.2.1 節的最低批次矩陣要求。

#### 3.2.3 押注檔位自動選組

模擬使用哪一組設定（RTP／Variant 模型＋OP Jackpot 檔）由**使用者設定的押注**決定，不由人工逐批對照：

- **檔位判定**：`bet_tier_amount`＝單注金額（NB／EB＝實際押注；BF／SF＝購買價 ÷ Feature 倍數），`small < $2 ≤ medium ≤ $100 < big`。
- **自動對應**：使用者只設定「水位系列＋押注＋Bet Mode（＋Profile）」，程式依遊戲的檔位對應表自動選 `config_rtp_file` 與 OP Jackpot 檔；對應表由各遊戲定義並記錄於該遊戲的 `BATCH_RUNS` 註解或修改紀錄。
- 手動在批次中指定 `config_rtp_file` 時以手動值為準（override），並照常於報表 Base Info 印出實際使用的 config。
- Newbie 若各版權重共用，則不分檔位、固定一組。

範例（H028 雷神爆金，老手）：

| 檔位 | 系列 A（92／94 水位） | 系列 B（88／90 水位） | OP Jackpot |
|---|---|---|---|
| 小 Bet（< $2） | `config_94A.js` | `config_90B.js` | C |
| 中 Bet（$2 ~ $100） | `config_92A.js` | `config_88B.js` | B |
| 大 Bet（> $100） | `config_92A_Bet100.js` | `config_88B_Bet100.js` | B |

（Newbie 四版權重共用，不分檔位，OP Jackpot 用 A。）

> H028 已實作：`CONFIG_RTP_FILE = "AUTO"`（預設）＋`RTP_SERIES = "A"/"B"` 依押注自動選 config；`JACKPOT_FILE = "AUTO"` 依 Profile／檔位自動選 JP 檔（newbie→A、老手小→C、中大→B）。環境變數 `H028_RTP_SERIES` 可逐批覆寫；`BATCH_RUNS` 內明寫 `config_rtp_file` 即為 override。邊界已以 $1／$2／$100／$101／$150 實測（94A→92A→92A_Bet100 切換正確）。
...
- `rtp_total = rtp_link + rtp_bonus + rtp_game`；三個分項的歸類必須依數學文件，且不得重複計算。
...
- [ ] 報表檔名依 H026 規則排列，時間、Bet Mode、場次段均正確，且不含 `card` 後綴。
...
- [ ] 各批次皆滿足 `rtp_total = rtp_link + rtp_bonus + rtp_game`，並符合《[數學模型規範](數學模型規範.md)》第 1.4 節目標。
...
兩種 Config 都是 Simulator 與 Demogame 的執行參數，不是人工獨立維護的第二份數學文件。正式參數必須由各自對應的 `Source/*.xlsx` 轉出；轉換一律由 AI 依 mapping 執行，專案內不建立、不維護 Excel↔Config 轉檔工具。
...
依文件類型更新 1 碼或 4 碼版本
      ↓
由 AI 依 Source 與 mapping 轉出 Config
      ↓
檢查欄位、型別、長度、權重與版本
      ↓
...
禁止直接修改 Config 中由 XLSX 產生的大型陣列來規避轉檔；結果錯誤時應修正 XLSX 或 mapping 後重新轉出。
...
- [ ] Config 可由 Source 重新轉出，且不會產生非預期差異。
