...
def working_change_summary(config: dict[str, Any]) -> str:
    ...
    if major == "6":
        ...
        return "\n".join([
            "本報告比較的是專案根目錄的 **H016 v6 自然數學**；v6.3 已完整封存在 `Versions/6.3`，目前 Card System 版本為 **6.4.0.0**。",
            "",
            "- v6.2 將 `sf_1／sf_2／sf_3` 的物理輪帶與停輪權重分別同步 `fg_1／fg_2／fg_3`，SF 初始與 Retrigger 選表同步為 `0／6500／3500`；SF 掉落、Random Wild 與金框參數不變。",
            ...
            "- v6.2 BG／FG 使用 Card-Off 10 億場報表重算，BF 使用 v6.1 封存的原始 BF count/pay；自然機率 ≤0.07% 的區間停用。老手 92A 為 BG 65%、FG 27%、Total 92%；老手 94A 為 BG 65%、FG 29%、Total 94%；BF 92.5%，FG 週期 130 場。",
            "- v6.3 使用 `H0161_06_2608190046_betmode3_108.xlsx` 的 1 億場 Card-Off SF 資料重算；92A／94A SF 使用完全相同權重，RTP 硬鎖 92.5%。",
            "- v6.4 只修正新手 Normal Bet：92A／94A 均為 BG 65%、FG 28%、Total 93%；老手、BF、SF、自然輪帶與 FG 週期不變。",
            "- SF 線型以 `Super_Ace_feature_buy_2.jsonl` 的競品 Hit Rate 為基準；`(350,400]` 與 `(450,500]` 額外 ×1.5。最低有權重區間為 `(50,60]`，自然機率 ≤0.07% 的區間停用。",
            "- SF Card-On 100,000 場同 seed 驗證：92A／94A 結果完全相同，RTP 92.2565%、每 Free Spin Hit Rate 40.9317%、平均 Retry 67.32、Retry Limit Exceeded = 0。",
            "- 92A 老手 Card-On 1,000,000 場舊驗證：RTP 91.7553%（BG 64.7090%、FG 27.0464%）、FG Hit Rate 38.3726%、週期 128.67 場。",
            "- v6.4 新手 Card-On 1,000,000 場同 seed 驗證：92A／94A 結果完全相同，RTP 92.8454%（BG 65.0279%、FG 27.8175%）、FG Hit Rate 39.0264%、週期 130.57 場，Retry Limit Exceeded = 0；試算表理論值為 93.0000%。",
            "- 94A 老手理論值維持 BG 65%、FG 29%、Total 94%；既有 Card-On 1,000,000 場抽樣為 94.6689%。",
            "- BF Card-On 100,000 場確認 RTP 92.4725%、每 Free Spin Hit Rate 38.6208%、Retry Limit Exceeded = 0；理論 RTP 為 92.4999991%。",
