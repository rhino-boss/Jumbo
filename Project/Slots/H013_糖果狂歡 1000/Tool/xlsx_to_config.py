...
DEFAULT_SOURCE = SOURCE_DIR / "H013192.xlsx"
...
# Keep these names for the existing H013 Simulator and DemoGame adapters.
LEGACY_STRIP_NAMES = [
    "BG_strip",
    "BG_strip (2)",
    "BG_strip (3)",
...
def parse_parameter_block(ws: Any, start_row: int, name: str) -> dict[str, Any]:
    table_weights = {
        "normal": [to_int(ws.cell(row, 3).value) for row in range(start_row + 3, start_row + 6)],
        "extrabet": [to_int(ws.cell(row, 4).value) for row in range(start_row + 3, start_row + 6)],
    }
...
def parse_parameter(ws: Any) -> dict[str, dict[str, Any]]:
    ...
    for row in range(1, ws.max_row + 1):
        ...
        name = base_name if occurrences[base_name] == 1 else f"{base_name}_{occurrences[base_name]}"
        blocks[name] = parse_parameter_block(ws, row, name)
...
def build_config(source_path: Path) -> dict[str, Any]:
    ...
    model_match = re.fullmatch(r"H0131(?P<rtp>\d{2})(?P<variant>[A-Za-z0-9_-]*)", overview["model"])
    if not model_match:
        raise ValueError(f"Overview model {overview['model']!r} does not match H0131xx naming")
...
def validate_config(config: dict[str, Any], source_path: Path) -> None:
    ...
    filename_match = re.fullmatch(r"H0131(?P<rtp>\d{2})[A-Za-z0-9_-]*\.xlsx", source_path.name, re.IGNORECASE)
...
def derive_output_path(source_path: Path) -> Path:
    match = re.fullmatch(r"H0131(?P<rtp>\d{2})(?P<variant>[A-Za-z0-9_-]*)", source_path.stem, re.IGNORECASE)
    if not match:
        raise ValueError(f"Unsupported xlsx name: {source_path.name}; expected H013192.xlsx or H013194.xlsx")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build H013 config JSON/JS from H013192.xlsx and H013194.xlsx")
    parser.add_argument("--source", "--xlsx", dest="source", type=Path, default=DEFAULT_SOURCE)
    parser.add_argument("--output", type=Path)
    parser.add_argument("--all", action="store_true", help="process every Source/H0131*.xlsx workbook")
    parser.add_argument("--check", action="store_true", help="compare generated data with existing output files")
    args = parser.parse_args()

    sources = (
        sorted(path for path in SOURCE_DIR.glob("H0131*.xlsx") if not path.name.startswith("~$"))
        if args.all
        else [args.source.resolve()]
    )
    if not sources:
        raise FileNotFoundError(f"No H0131*.xlsx files found in {SOURCE_DIR}")
