"""Build H999 runtime data from the H999 PARsheet layout.
...
def write_js(config: dict[str, Any], output_path: str | Path) -> Path:
    ...
    output.write_text(
        "/* Generated from the H999 source xlsx. Do not edit by hand. */\n"
        f"window.H999_CONFIG={payload};\n",
        encoding="utf-8",
    )
    return output
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate H999 browser config from H999 xlsx")
