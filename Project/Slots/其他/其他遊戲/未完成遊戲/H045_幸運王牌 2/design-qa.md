# H045 幸運王牌2 — Index UI/UX 與動畫 QA

## 比對基準

- Source of truth：`Project/Slots/H015_賞金列車/index.html`
- Implementation：`Project/Slots/H045_幸運王牌 2/index.html`（runtime 已內嵌）
- 完整畫面比對：`QA/h015-H045-comparison.png`
- H015 基準截圖：`QA/h015-reference.png`
- H045 實作截圖：`QA/H045-images.png`（已套用專屬圖片）
- H045 互動結果：`QA/H045-interaction.png`
...
- BG / FG 倍數僅依有效中獎 Cascade 次序逐階提升，沒有跳階起始機制。

## 五個視覺面向

- Composition：頂部標題與統計、中央盤面、右側控制、下方設定／Help 的區塊順序與 H015 一致。
- Layout：欄距、面板寬度、按鈕排列、資訊密度與 H015 一致；H045 盤面依規則保持 5 × 4。
...
- 無 P0、P1、P2 問題。
- 預期差異：H045 為 5 × 4／1024 Ways；H015 為不同盤面結構。
- 預期差異：H045 僅有 Normal Bet 與 Buy Feature 兩種模式，不提供 Buy Super Feature。
- 預期差異：依需求固定上排倍數，不套用 H015 的滾動倍數行為。
- H045 已換用專屬 symbol 素材：M1–M8、G1–G8、W1/W2 與 C1；不再依賴 H015 圖片。
...
- Simulator 設定解析新增 H045 身分與必要欄位驗證，避免 Notebook／不同工作目錄誤載同名 config。
- CLI 指定 rounds 或 bet mode 時不再被 `RUN_ALL_COMBINATIONS` 改成批次執行。
- 倍數流程統一為只依有效中獎 Cascade 次序推進。
- 2026-08-07：移除 Buy Super Feature 按鈕與 Super Free Game 分支；PARsheet ID 改為 H045192 / H045194。
- 2026-08-07：Free Game 表組合改由卡片的「使用的表」代碼（D / E）選組，不再對 D、E 做 50/50 抽取。
- 2026-08-07：修正 Cascade 掉落後金框位置索引未重新對應的問題（可能誤把 WW2 蓋到 C1）。
