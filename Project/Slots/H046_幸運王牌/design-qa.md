# H046 幸運王牌 — Index UI/UX 與動畫 QA

## 比對基準

- Source of truth：`Project/Slots/H015_賞金列車/index.html`
- Implementation：`Project/Slots/H046_幸運王牌/index.html` + `h046_runtime.js`
- 完整畫面比對：`QA/h015-h046-comparison.png`
- H015 基準截圖：`QA/h015-reference.png`
- H046 實作截圖：`QA/h046-images.png`（已套用專屬圖片）
- H046 互動結果：`QA/h046-interaction.png`
...
- 上排倍數依需求固定為 ×1、×2、×3、×5，不執行 H015 的倍數滾動動畫。

## 五個視覺面向

- Composition：頂部標題與統計、中央盤面、右側控制、下方設定／Help 的區塊順序與 H015 一致。
- Layout：欄距、面板寬度、按鈕排列、資訊密度與 H015 一致；H046 盤面依規則保持 5 × 4。
...
- 無 P0、P1、P2 問題。
- 預期差異：H046 為 5 × 4／1024 Ways；H015 為不同盤面結構。
- 預期差異：H046 多一個 Buy Super Feature 按鈕。
- 預期差異：依需求固定上排倍數，不套用 H015 的滾動倍數行為。
- H046 已換用專屬 symbol 素材：M1–M8、G1–G8、W1/W2 與 C1；不再依賴 H015 圖片。
...
- Simulator 設定解析新增 H046 身分與必要欄位驗證，避免 Notebook／不同工作目錄誤載同名 config。
