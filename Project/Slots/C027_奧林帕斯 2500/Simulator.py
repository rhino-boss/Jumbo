...
CONFIG_FILE = os.environ.get("C027_CONFIG_FILE", CONFIG_FILE)
CONFIG_RTP_FILE = os.environ.get("C027_CONFIG_RTP_FILE", CONFIG_RTP_FILE)
TOTAL_ROUNDS = int(os.environ.get("C027_TOTAL_ROUNDS", TOTAL_ROUNDS))
BET_MODE = int(os.environ.get("C027_BET_MODE", BET_MODE))
BET_MULTI = float(os.environ.get("C027_BET_MULTI", BET_MULTI))
BASE_BET = float(os.environ.get("C027_BASE_BET", BASE_BET))
CARD_SYSTEM_ENABLED = parse_env_bool("C027_CARD_SYSTEM_ENABLED", CARD_SYSTEM_ENABLED)
CARD_SYSTEM_IS_NEWBIE = parse_env_bool("C027_CARD_SYSTEM_IS_NEWBIE", CARD_SYSTEM_IS_NEWBIE)
THREADS = int(os.environ.get("C027_THREADS", THREADS))
RANDOM_SEED = int(os.environ["C027_RANDOM_SEED"]) if os.environ.get("C027_RANDOM_SEED") else RANDOM_SEED
RUN_ALL_COMBINATIONS = parse_env_bool("C027_RUN_ALL_COMBINATIONS", RUN_ALL_COMBINATIONS)
OUTPUT_REPORT = parse_env_bool("C027_OUTPUT_REPORT", OUTPUT_REPORT)
SHOW_CONSOLE_SUMMARY = parse_env_bool("C027_SHOW_CONSOLE_SUMMARY", SHOW_CONSOLE_SUMMARY)
SHOW_CONSOLE_DETAIL = parse_env_bool("C027_SHOW_CONSOLE_DETAIL", SHOW_CONSOLE_DETAIL)
RUN_SINGLE_SPIN_DEBUG = parse_env_bool("C027_RUN_SINGLE_SPIN_DEBUG", RUN_SINGLE_SPIN_DEBUG)


def resolve_base_dir():
    override = os.environ.get("C027_BASE_DIR")
    candidates = []
    if override:
    ...
    candidates.extend([cwd, cwd / "Project_AI" / "Slots" / "C027_奧林帕斯 2500"])
    for parent in [cwd, *cwd.parents]:
        candidates.append(parent / "Project_AI" / "Slots" / "C027_奧林帕斯 2500")
        candidates.append(parent / "Slots" / "C027_奧林帕斯 2500")
    for candidate in candidates:
        candidate = candidate.resolve()
        if (candidate / CONFIG_FILE).is_file() and (candidate / CONFIG_RTP_FILE).is_file():
            return candidate
    raise FileNotFoundError(f"Cannot locate C027 base directory containing {CONFIG_FILE} and {CONFIG_RTP_FILE}")
...
def merge_runtime_config(natural, rtp):
    ...
    for profile_name in ("normal", "featurebuy"):
        if profile_name not in natural_parameter or profile_name not in rtp_parameter:
            continue
        for key in ("c2", "c3", "use_super_multiplier"):
...
RULE_DOCUMENT = str(CFG.get("rule_document", "game_rule.md"))
...
INITIAL_MULTIPLIER_COUNT = max(
    len(PARAMETER["super_multiplier"]["multipliers"]),
    max(len(PARAMETER[name]["c2"]["multipliers"]) for name in PROFILE_NAMES),
)
...
# Scene Mixture (C027): when `free_table.weights` carries positive weight the FG
# schedule stops being a fixed per-table spin count and every free spin draws its
# scene table independently.  Two scenes with different hit rate / ball rate can
# then be mixed to control the *joint* distribution (per-spin hit rate, ball
# presence, cascade depth) instead of only the pooled marginals.
FREE_TABLE_WEIGHT_CUM = np.zeros((PROFILE_COUNT, MAX_FREE_TABLES), dtype=np.int64)
FREE_MIX_ENABLED = np.zeros(PROFILE_COUNT, dtype=np.int64)
FREE_INITIAL_SPINS = np.zeros(PROFILE_COUNT, dtype=np.int64)
FREE_RETRIGGER_SPINS = np.zeros(PROFILE_COUNT, dtype=np.int64)
...
for profile_index, profile_name in enumerate(PROFILE_NAMES):
    ...
    mix_weights = free_table.get("weights")
    if mix_weights and sum(int(value) for value in mix_weights) > 0:
        if len(mix_weights) != len(free_table["names"]):
            raise ValueError(f"{profile_name}.free_table.weights length must match names")
        if any(int(value) < 0 for value in mix_weights):
            raise ValueError(f"{profile_name}.free_table.weights must not be negative")
        FREE_MIX_ENABLED[profile_index] = 1
        FREE_TABLE_WEIGHT_CUM[profile_index, : len(mix_weights)] = np.cumsum(
            np.asarray(mix_weights, dtype=np.int64)
        )
        FREE_INITIAL_SPINS[profile_index] = int(
            free_table.get("initial_spins", sum(int(v) for v in free_table["initial"]))
        )
        FREE_RETRIGGER_SPINS[profile_index] = int(
            free_table.get("retrigger_spins", sum(int(v) for v in free_table["retrigger"]))
        )
        if FREE_INITIAL_SPINS[profile_index] <= 0:
            raise ValueError(f"{profile_name}.free_table needs a positive initial spin count")
    ...
    C2_MULTIPLIERS[profile_index, : len(profile["c2"]["multipliers"])] = np.asarray(profile["c2"]["multipliers"], dtype=np.int64)
    super_block = PARAMETER["super_multiplier"]
    C3_MULTIPLIERS[profile_index, : len(super_block["multipliers"])] = np.asarray(super_block["multipliers"], dtype=np.int64)
    super_cumulative = np.asarray(super_block["weights_cum"][super_block["table_names"][0]], dtype=np.int64)
    for table_name in profile["c2"]["table_names"]:
        ...
        # A scene may carry its own Super Multiplier weights; the global
        # `super_multiplier` table stays the fallback so older configs still load.
        scene_c3 = profile.get("c3", {}).get("weights_cum", {}).get(table_name)
        if scene_c3 is not None and sum(int(value) for value in scene_c3) > 0:
            C3_WEIGHT_CUM[profile_index, table_id, : len(scene_c3)] = np.asarray(scene_c3, dtype=np.int64)
        else:
            C3_WEIGHT_CUM[profile_index, table_id, : len(super_cumulative)] = super_cumulative
...
# Multi-dimensional card conditions (C027).  A range card may additionally require
# the accepted result to have (or not have) a multiplier ball on the final board,
# and/or a cascade count inside a window.  Both are pure accept/reject predicates
# on an already-generated result, so the reel strips and paytable stay untouched.
CARD_BALL_ANY = 0
CARD_BALL_REQUIRED = 1
CARD_BALL_FORBIDDEN = 2
CARD_BALL_MODES = {"any": CARD_BALL_ANY, "with": CARD_BALL_REQUIRED, "without": CARD_BALL_FORBIDDEN}
CARD_BALL = np.zeros((len(CARD_PROFILE_LISTS), MAX_CARDS), dtype=np.int64)
CARD_COMBO_MIN = np.full((len(CARD_PROFILE_LISTS), MAX_CARDS), -1, dtype=np.int64)
CARD_COMBO_MAX = np.full((len(CARD_PROFILE_LISTS), MAX_CARDS), -1, dtype=np.int64)
for card_profile_index, cards in enumerate(CARD_PROFILE_LISTS):
    ...
    for card_index, card in enumerate(cards):
        ...
        ball_mode = str(card.get("ball", "any")).strip().lower()
        if ball_mode not in CARD_BALL_MODES:
            raise ValueError(f"card.ball must be any/with/without, got {ball_mode!r}")
        CARD_BALL[card_profile_index, card_index] = CARD_BALL_MODES[ball_mode]
        CARD_COMBO_MIN[card_profile_index, card_index] = int(card.get("combo_min", -1))
        CARD_COMBO_MAX[card_profile_index, card_index] = int(card.get("combo_max", -1))
...
def build_bet_context(bet_mode, bet_multi):
    ...
    return {
        ...
        # C027 has no product Max Win cap. The 2500 value limits one C2/C3
...
# Cross counters for the competitor report 5.2 (ball present x cascade count) and
# 5.1.1 (balls per spin).  Pure counters: they never change an outcome.
R_CASCADE_BG_BALL = 25
R_CASCADE_BG_NOBALL = 26
R_CASCADE_FG_BALL = 27
R_CASCADE_FG_NOBALL = 28
R_BALL_COUNT_BG = 29
R_BALL_COUNT_FG = 30
# Competitor report 5.3: FG hit rate and average win by the accumulated FG
# multiplier at the start of the spin.  Buckets 0..9 are 0-9, 10-19, ... 90-99 and
# bucket 10 is 100+.  Pure counters.
R_FG_CUM_TIER_SPINS = 31
R_FG_CUM_TIER_HITS = 32
R_FG_CUM_TIER_PAY = 33

FG_CUM_TIER_COUNT = 11

RECORD_COLS = max(128, len(THRESHOLD_RECORD), SYMBOL_COUNT, MULTIPLIER_LEVEL_COUNT)
RECORD_SIZE = (34, RECORD_COLS)
...
@njit(nogil=True)
def is_card_shape_match(card_profile_index, card_index, ball_count, cascades):
    """Extra card dimensions: ball presence and cascade window."""
    if card_index < 0:
        return True
    mode = CARD_BALL[card_profile_index, card_index]
    if mode == CARD_BALL_REQUIRED and ball_count <= 0:
        return False
    if mode == CARD_BALL_FORBIDDEN and ball_count > 0:
        return False
    combo_min = CARD_COMBO_MIN[card_profile_index, card_index]
    if combo_min >= 0 and cascades < combo_min:
        return False
    combo_max = CARD_COMBO_MAX[card_profile_index, card_index]
    if combo_max >= 0 and cascades > combo_max:
        return False
    return True
...
@njit(nogil=True)
def prepare_multiplier_symbol(symbol, profile_index, table_id, initial_ball_count):
    if symbol != C2:
        return symbol, 0
    bucket = min(6, max(1, initial_ball_count)) - 1
    denominator = USE_SUPER_DENOMINATOR[profile_index]
    use_c3 = 1 if denominator > 0 and np.random.randint(0, denominator) < USE_SUPER_WEIGHT[profile_index, table_id, bucket] else 0
...
def generate_board(table_id, profile_index):
    ...
    for row in range(WINDOW_SIZE):
        for reel in range(REEL_NUM):
            symbol, value = prepare_multiplier_symbol(board[row, reel], profile_index, table_id, initial_ball_count)
...
@njit(nogil=True)
def cascade_board(board, multiplier_values, table_id, profile_index, starts, drop_counts, winning, any_win, initial_ball_count):
    if any_win == 0:
        return
    for reel in range(REEL_NUM):
        ...
        while output_row >= 0:
            ...
            while symbol == C1 and has_scatter == 1:
                drop_counts[reel] += 1
                symbol = SYMBOL_IDS[pick_cumulative(DROP_WEIGHT_CUM[table_id, reel])]
            symbol, value = prepare_multiplier_symbol(symbol, profile_index, table_id, initial_ball_count)
            if symbol == C1:
                has_scatter = 1
...
def play_cluster_spin(table_id, profile_index, scene, bet_multi):
    ...
    for _ in range(CASCADE_LIMIT):
        ...
        if any_win == 0:
            break
        cascades += 1
        cascade_board(board, multiplier_values, table_id, profile_index, starts, drop_counts, winning, any_win, initial_ball_count)
...
@njit(nogil=True)
def append_free_tables_mixed(target, current_length, spins, weight_cum, table_ids):
    """Scene Mixture schedule: every appended free spin draws its own scene table."""
    for _ in range(spins):
        if current_length >= target.shape[0]:
            return current_length
        target[current_length] = table_ids[pick_cumulative(weight_cum)]
        current_length += 1
    return current_length


@njit(nogil=True)
def schedule_free_spins(target, current_length, profile_index, is_retrigger):
    if FREE_MIX_ENABLED[profile_index] == 1:
        spins = FREE_RETRIGGER_SPINS[profile_index] if is_retrigger == 1 else FREE_INITIAL_SPINS[profile_index]
        return append_free_tables_mixed(
            target, current_length, spins,
            FREE_TABLE_WEIGHT_CUM[profile_index], FREE_TABLE_IDS[profile_index],
        )
    counts = FREE_RETRIGGER_COUNTS[profile_index] if is_retrigger == 1 else FREE_INITIAL_COUNTS[profile_index]
    return append_free_tables(target, current_length, counts, FREE_TABLE_IDS[profile_index])
...
def run_free_game_session(record, profile_index, bet_mode, bet_multi, coin_in):
    ...
    scheduled = schedule_free_spins(free_tables, 0, profile_index, 0)
    spin_index = 0
    ...
    while spin_index < scheduled and spin_index < MAX_FREE_SPINS:
        ...
        tier = min(FG_CUM_TIER_COUNT - 1, cumulative_multiplier // 10)
        ...
        record[R_FG_CUM_TIER_SPINS, tier] += 1
        record[R_FG_CUM_TIER_HITS, tier] += 1 if fg_spin_pay > 0 else 0
        record[R_FG_CUM_TIER_PAY, tier] += fg_spin_pay
        ...
        # C2/C3 never cascade away, so the final-board count is every ball the spin saw
        if fg_c2_count > 0:
            record[R_CASCADE_FG_BALL, min(fg_cascades, RECORD_COLS - 1)] += 1
        else:
            record[R_CASCADE_FG_NOBALL, min(fg_cascades, RECORD_COLS - 1)] += 1
        record[R_BALL_COUNT_FG, min(fg_c2_count, RECORD_COLS - 1)] += 1
        ...
        if fg_scatter_count >= FG_RETRIGGER_COUNT and scheduled < MAX_FREE_SPINS:
            previous = scheduled
            scheduled = schedule_free_spins(free_tables, scheduled, profile_index, 1)
...
def simulator_chunk(total_round, bet_mode, bet_multi, random_seed):
    ...
    while accepted_rounds < total_round:
        ...
        if bg_c2_count > 0:
            record[R_CASCADE_BG_BALL, min(bg_cascades, RECORD_COLS - 1)] += 1
        else:
            record[R_CASCADE_BG_NOBALL, min(bg_cascades, RECORD_COLS - 1)] += 1
        record[R_BALL_COUNT_BG, min(bg_c2_count, RECORD_COLS - 1)] += 1
            ...
            if bet_mode == MODE_NORMALBET or bet_mode == MODE_EXTRABET:
                if bg_card_index >= 0 and CARD_TYPES[bg_card_profile, bg_card_index] == CARD_TYPE_FREE_GAME:
                    ...
                    if triggered_free_game == 0 or trigger_cap < 0 or bg_pay / card_coin_in > trigger_cap:
                        accepted = 0
                        fail_reason = 2
                elif (triggered_free_game == 1
                      or not is_card_match(bg_card_profile, bg_card_index, bg_pay, card_coin_in, 0)
                      or not is_card_shape_match(bg_card_profile, bg_card_index, bg_c2_count, bg_cascades)):
                    accepted = 0
                    fail_reason = 1
...
def build_result_frames(record, total_round, duration, coin_in, bet_mode, bet_multi):
    ...
    cascade_frame = pd.DataFrame(
        {
            ...
            # split by whether a multiplier ball was on the board (report 5.2)
            "BG_Ball_Count": values[R_CASCADE_BG_BALL, :20],
            "BG_NoBall_Count": values[R_CASCADE_BG_NOBALL, :20],
            "FG_Ball_Count": values[R_CASCADE_FG_BALL, :20],
            "FG_NoBall_Count": values[R_CASCADE_FG_NOBALL, :20],
            # balls per spin (report 5.1.1); index 0 = no ball
            "BG_Balls_Per_Spin": values[R_BALL_COUNT_BG, :20],
            "FG_Balls_Per_Spin": values[R_BALL_COUNT_FG, :20],
        }
    )
    ...
    tier_spins = values[R_FG_CUM_TIER_SPINS, :FG_CUM_TIER_COUNT]
    tier_hits = values[R_FG_CUM_TIER_HITS, :FG_CUM_TIER_COUNT]
    tier_pay = values[R_FG_CUM_TIER_PAY, :FG_CUM_TIER_COUNT]
    safe_spins = np.where(tier_spins > 0, tier_spins, 1)
    fg_tier_frame = pd.DataFrame(
        {
            "Cumulative_Multiplier": [f"{index * 10}-{index * 10 + 9}" for index in range(FG_CUM_TIER_COUNT - 1)] + ["100+"],
            "FG_Spins": tier_spins,
            "FG_Hits": tier_hits,
            "FG_Hit_Rate": np.where(tier_spins > 0, tier_hits / safe_spins, 0.0),
            # per-spin win expressed against the Normal Bet base cost, so Normal Bet
            # and Buy Feature reports stay on the same scale
            "FG_Avg_Win_X": np.where(tier_spins > 0, tier_pay / safe_spins / (DEFAULT_COIN_IN * NORMALBET * bet_multi), 0.0),
        }
    )
    return (summary, base_frame, multiplier_frame, symbol_frame, cascade_frame, c2_frame,
            scatter_frame, symbol_bucket_frame, fg_tier_frame)
...
def output_report(frames, record, bet_mode, total_round):
    (summary, base_frame, multiplier_frame, symbol_frame, cascade_frame, c2_frame,
     scatter_frame, symbol_bucket_frame, fg_tier_frame) = frames
    ...
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        ...
        fg_tier_frame.to_excel(writer, sheet_name="FG Cumulative Multiplier", index=False)
...
def run_batch_runs():
    ...
    for index, combo in enumerate(BATCH_RUNS, start=1):
        ...
        env = os.environ.copy()
        env["C027_CONFIG_FILE"] = combo["config_file"]
        env["C027_CONFIG_RTP_FILE"] = combo["config_rtp_file"]
        env["C027_BET_MODE"] = str(combo["bet_mode"])
        env["C027_TOTAL_ROUNDS"] = str(combo["total_rounds"])
        env["C027_CARD_SYSTEM_ENABLED"] = "true" if combo.get("card_system_enabled", CARD_SYSTEM_ENABLED) else "false"
        env["C027_CARD_SYSTEM_IS_NEWBIE"] = "true" if combo.get("card_system_is_newbie", CARD_SYSTEM_IS_NEWBIE) else "false"
        env["C027_BASE_BET"] = str(combo["base_bet"])
        env["C027_RUN_ALL_COMBINATIONS"] = "false"
        env["C027_BATCH_CHILD"] = "1"
        env["PYTHONUTF8"] = "1"
...
def main():
    if RUN_ALL_COMBINATIONS and os.environ.get("C027_BATCH_CHILD") != "1":
        run_batch_runs()
        return
    ...
    if os.environ.get("C027_BATCH_CHILD") != "1":
        current_batch = {
            "config_file": CONFIG_FILE,
            "config_rtp_file": CONFIG_RTP_FILE,
            "bet_mode": BET_MODE,
