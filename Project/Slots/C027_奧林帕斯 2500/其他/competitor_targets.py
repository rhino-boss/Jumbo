"""C027 competitor target extraction.
...
OTHER = ROOT / "其他"
COMPETITOR = OTHER / "競品資料"
HTML = COMPETITOR / "遊戲數據_Gates_of_Olympus_1000.html"
...
# §5.2.1 有球／無球的 Hit Rate 與 spin 數（含 Scatter 得分的總命中口徑）
BALL_CONDITIONAL = {
    "BG": {"with": {"spins": 249, "hit": 0.4016}, "without": {"spins": 8849, "hit": 0.2172}},
    "FG": {"with": {"spins": 166, "hit": 0.5060}, "without": {"spins": 166, "hit": 0.4096}},
    "BF": {"with": {"spins": 1816, "hit": 0.3601}, "without": {"spins": 2567, "hit": 0.4083}},
}
...
class Targets:
    ...
    def conditional_symbol_hit(self, scene: str) -> dict[str, float]:
        """§5.2.1 的有球／無球 Hit Rate 換算成「符號中獎」口徑.

        §5.2.1 是總命中（含只有 Scatter 得分、不需要消除的 spin），而輪帶只能控制
        「初始盤面有沒有符號中獎」。兩者的比例就是 §3.5.1 的 `1 - combo0` 除以
        §2.1／§5.1 的總 Hit Rate；把這個比例同時套在有球／無球兩列上，
        就得到輪帶可以直接對齊的目標。
        """
        block = BALL_CONDITIONAL[scene]
        spins_with = block["with"]["spins"]
        spins_without = block["without"]["spins"]
        total = spins_with + spins_without
        pooled = (spins_with * block["with"]["hit"] + spins_without * block["without"]["hit"]) / total
        scale = self.initial_hit_target(scene) / pooled if pooled > 0 else 1.0
        return {
            "with": block["with"]["hit"] * scale,
            "without": block["without"]["hit"] * scale,
            "ball_share_observed": spins_with / total,
            "scale": scale,
        }

    def interval_shape(self) -> dict[str, list[float]]:
        """64-interval competitor line shape, taken from 競品資料/競品比較_H027.md appendix A."""
        rows = []
        for line in (COMPETITOR / "競品比較_H027.md").read_text(encoding="utf-8").splitlines():
