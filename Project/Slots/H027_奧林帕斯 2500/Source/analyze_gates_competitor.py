...
DEFAULT_INPUT = PROJECT_DIR / "參考資料" / "game_responses-gates of olympus 1000.xlsx"
DEFAULT_BUY_INPUT = PROJECT_DIR / "參考資料" / "game_responses-gates of olympus 1000 - buyfeature.xlsx"
DEFAULT_MODEL = PROJECT_DIR / "Source" / "H0271.xlsx"
DEFAULT_REPORT = PROJECT_DIR / "分析報告_gates of olympus 1000.md"
DEFAULT_COMPARISON = PROJECT_DIR / "競品參考數值比較.md"
DEFAULT_METRICS = PROJECT_DIR / "參考資料" / "analysis_gates_of_olympus_1000_metrics.json"
...
def update_h0271_model(model_path: Path, analysis, buy_analysis=None):
    ...
    fg_phases = [[], [], []]
    for session in sessions:
        for index, spin in enumerate(session.fg_spins):
            fg_phases[min(index // 5, 2)].append(spin)
    trigger_spins = [session.bg for session in sessions if session.triggered_fg]
    buy_trigger_spins = (
        [session.bg for session in buy_analysis["_sessions"] if session.triggered_fg]
    ...
    overview["B2"] = "H0271"
    overview["B18"] = basic["rtp_bg"]
    overview["B19"] = basic["rtp_fg"]
    overview["D18"] = 1 / basic["hit_rate_bg"] if basic["hit_rate_bg"] else None
    for cell in ("B18", "B19", "C18"):
        overview[cell].number_format = "0.0000%"
    overview["D18"].number_format = "0.00"

    parameter = workbook["Parameter"]
    # All three BG tables share the same empirical targets; the table selector
    # therefore does not distort the competitor distribution.
    for row in range(4, 7):
        parameter.cell(row, 3).value = 1
    # Gates uses 15 initial spins and adds 5 on retrigger.  The three FG tables
    # represent spins 1-5, 6-10, and 11+ respectively.
    for row, initial_count, retrigger_count in ((11, 5, 2), (12, 5, 2), (13, 5, 1)):
        parameter.cell(row, 3).value = initial_count
        parameter.cell(row, 4).value = retrigger_count
    # The competitor has one multiplier-ball type.  Map all observed balls to
    # C2 and leave C3 selection disabled until a distinct C3 target exists.
    for row in range(18, 24):
        for column in range(3, 9):
            parameter.cell(row, column).value = 0

    observed_values = sorted(
        set(multiplier_value_counts(bg_spins))
        | set(multiplier_value_counts([spin for phase in fg_phases for spin in phase]))
    )
    ...
    fg_value_weights = [map_multiplier_weights(multiplier_value_counts(phase), levels) for phase in fg_phases]
    table_weight_rows = {
        9: bg_value_weights,
        10: bg_value_weights,
        11: bg_value_weights,
        12: fg_value_weights[0],
        13: fg_value_weights[1],
        14: fg_value_weights[2],
    }
    ...
    sheet_groups = {
        "BG_Symbol": bg_spins,
        "BG_Symbol (2)": bg_spins,
        "BG_Symbol (3)": bg_spins,
        "FG_Symbol": fg_phases[0],
        "FG_Symbol (2)": fg_phases[1],
        "FG_Symbol (3)": fg_phases[2],
        "BF_Symbol": buy_trigger_spins,
    }
    strip_fit = {}
    for index, (sheet_name, spins) in enumerate(sheet_groups.items()):
        ...
        strip_fit[sheet_name] = write_strip_sheet(
            ...
            force_scatter_first_four=sheet_name == "BF_Symbol",
            high_fraction=0.75 if sheet_name.startswith("BG_Symbol") else 0.70,
            scatter_scale=1.07 if sheet_name.startswith("BG_Symbol") else 1.0,
        )
    ...
    return {
        ...
        "fg_phase_samples": [len(phase) for phase in fg_phases],
        "buy_feature_trigger_samples": len(buy_trigger_spins),
        "strip_fit": strip_fit,
        "scope": "Overview, Parameter, 3 BG strips, 3 FG strips, and BF strip",
    }
...
{markdown_table(
    ...
    [
        ...
        ['Parameter / Free Game Table', f"FG 三階段樣本數為 {model_update['fg_phase_samples']}；初始 5／5／5、Retrigger 2／2／1"],
        ...
        ['BF_Symbol', f"使用競品 Buy Feature 觸發盤面 {model_update['buy_feature_trigger_samples']:,} 場；配合 Simulator，前 4 輪各保留至少 1 顆 C1"],
    ],
)}
...
def main():
    ...
    if not args.no_model_update:
        model_update = update_h0271_model(args.model.resolve(), analysis, buy_analysis)
