...
- FG 兩表的 8:7 加權初始符號 counts 與掉落 weights 由 `calibrate_fg_two_tables.py` 保持為競品 R1～R6 目標分布。
- 初始盤面的 `Scene × Reel × Symbol × Stack 1～5` 排列由 `calibrate_stack_arrangement.py` 校準；BG／FG 雙表工具在 `--write` 時會自動重套這個排列，避免後續調參破壞堆疊分布。
