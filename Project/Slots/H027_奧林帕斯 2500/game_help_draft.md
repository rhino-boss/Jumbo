# Olympus 2500 Game Help

遊戲類型：`Video Slot - Pay Anywhere / Cascade`

## WINS

### Pay Anywhere

| Item | 中文欄 | English |
| --- | --- | --- |
| 主要標題 | 中獎方式 | WINS |
| 副標題 | 全盤計數 | PAY ANYWHERE |
| 規則說明 | 盤面為 6 輪 × 5 列，共 30 格。同一一般符號在整個盤面出現 8 個以上即可得獎，不要求相鄰。 | THE GAME USES A 6-REEL BY 5-ROW GRID WITH 30 POSITIONS. EIGHT OR MORE MATCHING REGULAR SYMBOLS ANYWHERE ON THE GRID AWARD A WIN; ADJACENCY IS NOT REQUIRED. |
| 規則說明 | 同一盤面可有多種一般符號同時得獎；每種符號依其總數對應的賠率級距支付一次。 | MULTIPLE REGULAR SYMBOL TYPES MAY WIN ON THE SAME GRID. EACH SYMBOL TYPE PAYS ONCE ACCORDING TO ITS TOTAL COUNT. |
| 規則說明 | 本遊戲沒有 Wild。Scatter 與 Multiplier 不屬於一般得分符號。 | THIS GAME HAS NO WILD. SCATTER AND MULTIPLIER SYMBOLS ARE NOT REGULAR PAYING SYMBOLS. |

## PAYTABLE

### Regular Symbols

| Item | 中文欄 | English |
| --- | --- | --- |
| 主要標題 | 賠率表 | PAYTABLE |
| 副標題 | 一般符號 | REGULAR SYMBOLS |
| 規則說明 | 8–9／10–11／12+ 個：皇冠 10／25／50；沙漏 2.5／10／25；戒指 2／5／15；聖杯 1.5／2／12。 | 8–9 / 10–11 / 12+ SYMBOLS: CROWN 10 / 25 / 50; HOURGLASS 2.5 / 10 / 25; RING 2 / 5 / 15; GOBLET 1.5 / 2 / 12. |
| 規則說明 | 8–9／10–11／12+ 個：紅寶石 1／1.5／10；紫寶石 0.8／1.2／8；金寶石 0.5／1／5；綠寶石 0.4／0.9／4；藍寶石 0.25／0.75／2。 | 8–9 / 10–11 / 12+ SYMBOLS: RED GEM 1 / 1.5 / 10; PURPLE GEM 0.8 / 1.2 / 8; GOLD GEM 0.5 / 1 / 5; GREEN GEM 0.4 / 0.9 / 4; BLUE GEM 0.25 / 0.75 / 2. |
| 規則說明 | 上述數值為 Bet 倍數；一般符號基本獎金為「賠率 × Bet」。 | VALUES ARE MULTIPLES OF BET. A REGULAR SYMBOL WIN IS CALCULATED AS PAYTABLE VALUE × BET. |

### Scatter

| Item | 中文欄 | English |
| --- | --- | --- |
| 副標題 | Scatter 獎金 | SCATTER PAYS |
| 規則說明 | 4／5／6 個 Scatter 分別支付 3／5／100 × Bet。3 個 Scatter 不支付獎金。Scatter 獎金不乘 C2／C3 倍數；Scatter 也會打擊 OP Jackpot 符號。 | 4 / 5 / 6 SCATTERS PAY 3 / 5 / 100 × BET. THREE SCATTERS DO NOT PAY. SCATTER PAYS ARE NOT MULTIPLIED BY C2 OR C3; SCATTERS ALSO HIT OP JACKPOT SYMBOLS. |

## CASCADING FEATURE

### Cascade Sequence

| Item | 中文欄 | English |
| --- | --- | --- |
| 主要標題 | 消除特色 | CASCADING FEATURE |
| 副標題 | 消除與補位 | CLEAR AND DROP |
| 規則說明 | 一般符號得獎後，所有中獎符號會先消除；新符號由上方補入並向下掉落，再重新判定得獎。 | AFTER A REGULAR SYMBOL WIN, ALL WINNING SYMBOLS ARE CLEARED FIRST. NEW SYMBOLS ENTER FROM ABOVE, DROP INTO PLACE, AND THE GRID IS EVALUATED AGAIN. |
| 規則說明 | Cascade 會持續進行，直到盤面不再形成新的得獎。 | CASCADES CONTINUE UNTIL THE GRID NO LONGER FORMS A NEW WIN. |

## MULTIPLIER

### C2 / C3 Multiplier

| Item | 中文欄 | English |
| --- | --- | --- |
| 主要標題 | 倍數符號 | MULTIPLIER |
| 副標題 | C2／C3 倍數 | C2 / C3 MULTIPLIER |
| 規則說明 | C2 的初始倍數為 2x、3x、4x、5x、6x 或 8x。C3 的初始倍數至少為 10x，並可經由消除升級，單顆最高為 2500x。 | C2 STARTS AT 2X, 3X, 4X, 5X, 6X, OR 8X. C3 STARTS AT 10X OR HIGHER AND MAY UPGRADE THROUGH WINNING CLEARS, UP TO 2500X PER SYMBOL. |
| 規則說明 | C2 維持出現時的倍數。每次發生中獎消除，消除前已在盤面的每顆 C3 升一個級距；本次補入的新 C3 要到下一次消除才升級。 | C2 KEEPS ITS ORIGINAL VALUE. EACH WINNING CLEAR UPGRADES EVERY C3 ALREADY ON THE GRID BY ONE LEVEL. A NEW C3 THAT DROPS IN IS NOT UPGRADED UNTIL A LATER CLEAR. |
| 規則說明 | C2 與 C3 不會被消除。同一 Spin 最終盤面的所有 C2／C3 倍數相加；沒有倍數符號時使用 1x。 | C2 AND C3 SYMBOLS ARE NOT CLEARED. ALL FINAL C2 AND C3 VALUES ARE ADDED; IF NO MULTIPLIER SYMBOL IS PRESENT, 1X APPLIES. |

## FREE GAME

### Trigger and Retrigger

| Item | 中文欄 | English |
| --- | --- | --- |
| 主要標題 | 免費遊戲 | FREE GAME |
| 副標題 | 觸發 | TRIGGER |
| 規則說明 | Base Game 的最終盤面出現 4 個以上 Scatter 時，觸發 15 次 Free Game。觸發判定在所有 Cascade 結束後進行。 | FOUR OR MORE SCATTERS ON THE FINAL BASE GAME GRID AWARD 15 FREE GAME SPINS. THE TRIGGER IS EVALUATED AFTER ALL CASCADES END. |
| 規則說明 | Free Game 最終盤面出現 3 個以上 Scatter 時加 5 Spins；總場次上限為 50 Spins。 | THREE OR MORE SCATTERS ON A FINAL FREE GAME GRID ADD 5 SPINS, UP TO A MAXIMUM OF 50 TOTAL SPINS. |
| 規則說明 | Free Game Spin 必須同時有一般符號得分，且最終盤面有 C2／C3，該次球倍率才會加入累積倍數。 | A FREE GAME SPIN MUST HAVE BOTH A REGULAR-SYMBOL WIN AND AT LEAST ONE C2 OR C3 ON ITS FINAL GRID BEFORE THE CURRENT BALL VALUES ARE ADDED TO THE CARRYING MULTIPLIER. |
| 規則說明 | 累積倍數只在上述有得分且有球的 Spin 作用；所有 Cascade 結束後，對該 Spin 的一般符號總分乘一次。 | THE CARRYING MULTIPLIER APPLIES ONLY ON SUCH A SCORING SPIN WITH A FINAL C2 OR C3. AFTER ALL CASCADES END, IT IS APPLIED ONCE TO THAT SPIN'S TOTAL REGULAR-SYMBOL WIN. |
| 規則說明 | 有一般符號得分但最終盤面沒有 C2／C3 時，本次以 1x 結算；既有累積倍數保留，但本次不作用。 | IF A SPIN HAS A REGULAR-SYMBOL WIN BUT NO FINAL C2 OR C3, IT PAYS AT 1X. THE EXISTING CARRY IS RETAINED BUT DOES NOT APPLY TO THAT SPIN. |
| 規則說明 | 只有 Scatter 得分或沒有得分時，球倍率不累積。累積倍數跨 Spin 保留至整場 Free Game 結束；Scatter 得分不乘倍數。 | BALL VALUES ARE NOT COLLECTED ON A SCATTER-ONLY OR NO-WIN SPIN. THE CARRY PERSISTS UNTIL THE FREE GAME ENDS, AND SCATTER PAYS ARE NOT MULTIPLIED. |

## BET MODES

### Extra Bet and Buy Feature

| Item | 中文欄 | English |
| --- | --- | --- |
| 主要標題 | 押注模式 | BET MODES |
| 副標題 | Extra Bet | EXTRA BET |
| 規則說明 | Extra Bet 扣款為 2 × 玩家所選基礎 Bet，獎金仍以該基礎 Bet 計算；Free Game 觸發機率目標為 Normal Bet 的 5 倍。 | EXTRA BET COSTS 2 × THE SELECTED BASE BET, WHILE WINS ARE STILL CALCULATED FROM THAT BASE BET. IT TARGETS FIVE TIMES THE NORMAL BET FREE GAME TRIGGER CHANCE. |
| 副標題 | Buy Feature | BUY FEATURE |
| 規則說明 | Buy Feature 扣款為 100 × 玩家所選基礎 Bet，FG 獎金仍以該基礎 Bet 計算；使用保證觸發盤面進入一般 Free Game。本遊戲不提供 Super Feature。 | BUY FEATURE COSTS 100 × THE SELECTED BASE BET, AND FREE GAME WINS ARE CALCULATED FROM THAT BASE BET. IT USES A GUARANTEED TRIGGER GRID TO ENTER THE REGULAR FREE GAME. SUPER FEATURE IS NOT AVAILABLE. |
