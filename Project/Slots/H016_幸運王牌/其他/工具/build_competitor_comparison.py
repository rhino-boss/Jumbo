...
def working_change_summary(config: dict[str, Any]) -> str:
    ...
    if major == "6":
        ...
        return "\n".join([
            "本報告比較的是專案根目錄的 **H016 v6 自然數學**；v6.2 已完整封存在 `Versions/6.2`，目前 Card System 版本為 **6.3.0.1**。",
            "",
            ...
            "- v6.2 BG／FG 使用 Card-Off 10 億場報表重算，BF 使用 v6.1 封存的原始 BF count/pay；自然機率 ≤0.07% 的區間停用。92A 新手／老手為 BG 65%、FG 27%、Total 92%；94A 新手／老手為 BG 65%、FG 29%、Total 94%；BF 92.5%，FG 週期 130 場。",
            ...
            "- 92A Card-On 1,000,000 場確認：老手 RTP 91.7553%（BG 64.7090%、FG 27.0464%）、FG Hit Rate 38.3726%、週期 128.67 場；新手 RTP 91.8907%（BG 64.8722%、FG 27.0186%）、FG Hit Rate 39.1050%、週期 129.90 場；Retry Limit Exceeded 均為 0。",
            "- 94A 理論值為老手／新手 BG 65%、FG 29%、Total 94%；Card-On 1,000,000 場抽樣：老手 94.6689%、新手 94.2503%，Retry Limit Exceeded 均為 0。",
