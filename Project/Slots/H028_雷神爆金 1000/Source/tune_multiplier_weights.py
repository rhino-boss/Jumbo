...
def natural_arrays(normal_base, normal_rows):
    ...
    # Detail!B163:C226 is an array reference to B86:C149. Buy Feature
    # therefore uses the same FG-session natural distribution and average
    # multipliers as the selected Normal Bet report.
    bf_counts = list(fg_counts)
    bf_pays = list(fg_pays)
    bf_rates = list(fg_rates)
    bf_averages = list(fg_averages)
    return {
...
def tune_variant(config_path, workbook_path, rtp_code, arrays, version):
    ...
    # The workbook is the source of truth. Re-export its integer odds
    # representation so the versioned config is exactly what XLSX produces.
    config = model_sync.build_config(workbook_path)
    model_sync.write_js_config(config_path, config)
...
def main():
    ...
    parser.add_argument(
        "--bf-report",
        type=Path,
        help="Deprecated: BF uses the Normal Bet FG distribution referenced by Detail formulas",
    )
    parser.add_argument("--version", default="2.0.0.37")
    parser.add_argument(
        "--config-suffix",
        default="",
        help="Append a version suffix to generated configs, for example 20037",
    )
    args = parser.parse_args()
    normal_base, normal_rows = report_data(args.normal_report.resolve())
    arrays = natural_arrays(normal_base, normal_rows)

    suffix = f"_{args.config_suffix.strip()}" if args.config_suffix.strip() else ""
    targets = (
        (92, PROJECT_DIR / f"config_92A{suffix}.js", BASE_DIR / "H028192A.xlsx"),
        (94, PROJECT_DIR / f"config_94A{suffix}.js", BASE_DIR / "H028194A.xlsx"),
    )
    output = {}
