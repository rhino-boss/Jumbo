...
倍率線型的區間上限必須與 H026 Simulator 一致，不可由各遊戲自行簡化。固定使用：
...
每個區間標籤採 `(前一上限, 當前上限]`，第一個區間為 `0`；`9999999` 作為最後收尾區間上限。
...
輸出檔名必須以 config 內記載的 `parsheet_id` 作為第一段，不可使用 `Game ID` 或 `display_name`。其餘建議包含：
...
* `H0281_YYMMDDHHMM_betmode0.xlsx`
