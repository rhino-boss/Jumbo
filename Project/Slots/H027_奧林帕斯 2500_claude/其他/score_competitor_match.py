...
# a scene can be produced by more than one strip; score the scene-defining one
SCENE_STRIP = {"BG": "BG_Symbol (2)", "FG": "FG_Symbol", "BF": "FG_Symbol (2)"}
...
def strip_structure(config: dict) -> dict:
    """Per-reel symbol shares, drop shares and window stack distribution per scene."""
    ...
    out = {}
    for scene, name in SCENE_STRIP.items():
        strip = by_name[name]
        length = len(strip["symbols"])
        initial = {}
        overall = np.zeros(length * 6, dtype=np.int64)
        cursor = 0
        for reel_index, reel in enumerate(REELS):
            ...
            overall[cursor:cursor + length] = longest
            cursor += length
        drop = {}
        for reel_index, reel in enumerate(REELS):
        ...
        out[scene] = {
            "initial": initial,
            "drop": drop,
            "stack": (np.bincount(overall, minlength=6)[1:6] / (length * 6)).tolist(),
        }
    return out
...
if __name__ == "__main__":
    ...
    parser.add_argument("--label", type=float, default=0.92)
