# H0151*.xlsx → config_*.js 欄位對照

`H015192.xlsx` 產生 `config_92.js`；`H015194.xlsx` 產生 `config_94.js`。來源檔名、模型 ID 與 RTP 會分別寫入 config，避免 94 版誤標為 92 版。
...
- `B11` → `normalbet`
- `B12` → `featurebuy`
- `C11 / C12` → `rtp_targets.normal / featurebuy`
...
- 雙擊 `Source/update_config.bat`：可選擇單一 xlsx，留空或輸入 `ALL` 會更新所有 `H0151*.xlsx`。
- 命令列執行 `python Source/xlsx_to_config.py --all`：重建所有 config。
- 命令列加上 `--check`：只驗證 xlsx 與 config 是否一致，不覆寫檔案。
