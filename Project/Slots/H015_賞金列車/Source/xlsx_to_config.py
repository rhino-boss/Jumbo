...
DEFAULT_XLSX = SOURCE_DIR / "H015192.xlsx"
...
def parse_overview(ws: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "rtp_targets": {
            "normal": to_float(ws["C11"].value),
            "featurebuy": to_float(ws["C12"].value),
        },
...
def generate_config(xlsx_path: Path, template: dict[str, Any]) -> dict[str, Any]:
    ...
    config: dict[str, Any] = {
        ...
        "source_xlsx": f"Source/{xlsx_path.name}",
        "source_box": f"Source/{xlsx_path.name}",
...
def derive_output_path(xlsx_path: Path) -> Path:
    match = re.fullmatch(r"H0151(?P<rtp>\d{2})(?P<variant>[A-Za-z0-9_-]*)", xlsx_path.stem, re.IGNORECASE)
    if not match:
        raise ValueError(f"Unsupported xlsx name: {xlsx_path.name}; expected a name such as H015192.xlsx")
...
def process_source(xlsx_path: Path, output_path: Path, template_path: Path | None, check: bool) -> None:
    template = load_template(template_path if template_path is not None else output_path)
    config = generate_config(xlsx_path, template)
    if check:
        if not output_path.exists():
            raise FileNotFoundError(f"Config not found: {output_path}")
        current = load_template(output_path)
        if comparable(config) != comparable(current):
            changed = sorted(key for key in set(config) | set(current) if comparable(config).get(key) != comparable(current).get(key))
            raise ValueError(f"Config mismatch for {output_path.name}: {changed}")
        print(f"Config is current: {output_path.name} <- {xlsx_path.name}")
        return
    write_config(output_path, config)
    if comparable(load_template(output_path)) != comparable(config):
        raise ValueError(f"Write verification failed: {output_path}")
    print(f"Generated {output_path.name} from {xlsx_path.name}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate H015 config files from H0151*.xlsx")
    parser.add_argument("--xlsx", type=Path, default=DEFAULT_XLSX)
    parser.add_argument("--output", type=Path)
    parser.add_argument("--template", type=Path)
    parser.add_argument("--all", action="store_true", help="Convert every H0151*.xlsx in Source")
    parser.add_argument("--check", action="store_true", help="Verify generated data without rewriting config")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    sources = sorted(path for path in SOURCE_DIR.glob("H0151*.xlsx") if not path.name.startswith("~$")) if args.all else [args.xlsx.resolve()]
    if not sources:
        raise FileNotFoundError(f"No H0151*.xlsx files found in {SOURCE_DIR}")
    if args.output and len(sources) != 1:
        raise ValueError("--output can only be used with one xlsx")
    for xlsx_path in sources:
        output_path = args.output.resolve() if args.output else derive_output_path(xlsx_path)
        template_path = args.template.resolve() if args.template else None
        process_source(xlsx_path, output_path, template_path, args.check)
