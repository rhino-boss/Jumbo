...
| 段位 | 變更時機 |
|---|---|
| 第 1 碼：遊戲參數 | 輪帶、符號權重、Pay Table、Feature、Bet Mode、RTP、觸發率、判獎或其他會改變遊戲數學結果的參數。 |
| 第 2 碼：卡片權重 | Newbie／Oldhand、BG／FG／Buy／Super Feature 的卡片區間或權重。 |
| 第 3 碼：SCR | SCR 相關設定、內容或結果有調整。 |
| 第 4 碼：其他文件 | 不影響前三類的規則文件、Help、說明、排版或其他文件異動。 |

### 3.2 遞增與歸零規則

| 異動類型 | 原版本 | 新版本 |
|---|---|---|
| 遊戲參數調整 | `1.2.3.4` | `2.0.0.0` |
| 卡片權重調整 | `1.2.3.4` | `1.3.0.0` |
| SCR 調整 | `1.2.3.4` | `1.2.4.0` |
| 其他文件異動 | `1.2.3.4` | `1.2.3.5` |

- 第 1 碼遞增後，第 2～4 碼全部歸零。
- 第 2 碼遞增後，第 3～4 碼全部歸零。
- 第 3 碼遞增後，第 4 碼歸零。
- 第 4 碼遞增時，前三碼不變。
- 同一次修改涉及多個類型時，只遞增順位最高的一碼，並將其後各碼歸零；順位為第 1 碼最高、第 4 碼最低。
- 每次修改前必須先檢查實際異動檔案與內容，再判斷所屬類型；不得只依檔名決定版本。
- 只要文件內容有異動，至少必須遞增第 4 碼；若該文件異動同時反映遊戲參數、卡片權重或 SCR 改動，則依較高順位遞增。
...
- 只修改文件、Help、排版或說明時，遞增第 4 碼。
- 修改會影響結果、RTP、觸發率、最大獎或 Retry 的遊戲參數時，遞增第 1 碼並重新模擬。
- 只調整 Card System 區間或權重時，遞增第 2 碼並重新模擬所有受影響 Profile 與 Bet Mode。
- SCR 有調整時，遞增第 3 碼並重新執行 SCR 對應檢查。

### 3.4 Index 版本顯示

- Index 的 Version 一律顯示完整四碼，例如 `1.2.3.4`，不得只顯示 `1.2`。
- 第 1、2 碼相同的版本，在 Index Version 選單中只能保留一個。
- 同組版本取第 3、4 碼數值最大的最新版本；其他同組版本不得出現在 Index。
- 例如同時存在 `1.2.0.3`、`1.2.1.0`、`1.2.1.4` 時，Index 只顯示 `1.2.1.4`。
- 不顯示的版本仍可保留在 `Versions/` 作為歷史檔，但不得出現在 Version 選單。
...
- Demo Game 的 Version 選單只能列出 manifest 中實際存在的版本與 Config，並套用 3.4 節的最新版本過濾規則。
