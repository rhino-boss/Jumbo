...
# row_data = pd.read_excel("03_老手救援/output_10w_2k.xlsx", header=None).to_numpy()
row_data = pd.read_excel("03_老手救援/output_10w_2k (增加新手資料).xlsx", header=None).to_numpy()
...
# rescue setting old
trigger_range = 150
observe_rtp_range_1 = 100

return_percnetage = 0.70  # 70
return_rtp_threshold_1 = 0.65  # 65

rescue_value_ceiling = 100
rescue_value_floor = 15

no_fg_trigger = 2000  # 可以訂2x理論週期
limit_continue_trigger = 0  # 避開N把內連續救援
    ...
    for i in range(player):
        ...
        for j in range(total_round):
            ...
            if cnt_spin == cnt_cum_range_trigger:
                ...
                if rtp_1 < return_rtp_threshold_1:
...
print("\n--- overview ---")
rtp = row_data_pay.sum() / total_round / player / threshold
rtp_rescue = record_data_rescue_pay.sum() / total_round / player / threshold
rtp_rescue_1 = (record_data_rescue_pay[record_data_rescue_mode == 1].sum() - row_data_pay[record_data_rescue_mode == 1].sum()) / total_round / player / threshold
rtp_rescue_2 = (record_data_rescue_pay[record_data_rescue_mode == 2].sum() - row_data_pay[record_data_rescue_mode == 2].sum()) / total_round / player / threshold
rtp_rescue_3 = (record_data_rescue_pay[record_data_rescue_mode == 3].sum() - row_data_pay[record_data_rescue_mode == 3].sum()) / total_round / player / threshold
...
total_can_trigger = (total_round - observe_rtp_range_1) / trigger_range * player
