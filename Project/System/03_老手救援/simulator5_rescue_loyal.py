...
# row_data = pd.read_excel("Project/System/03_老手救援/rowdata_2k_2k_模擬資料_增加新手期.xlsx", header=None).to_numpy()
row_data = pd.read_excel("Project/System/03_老手救援/rowdata_2k_2k_真實資料_包含新手期.xlsx", header=None).to_numpy()
...
return_percnetage_long_bad = 0.8 - 0.1
return_percnetage_long_good = 0.7 - 0.1
return_percnetage_before500 = 0.7 - 0.1

return_rtp_threshold_1_long_bad = 0.7
return_rtp_threshold_1_long_good = 0.6
return_rtp_threshold_1_before500 = 0.6
return_rtp_threshold_2 = 0.75
return_rtp_threshold_3 = 0.9

rescue_value_long_bad_ceiling = 100
rescue_value_long_bad_floor = 1
rescue_value_long_good_ceiling = 15
rescue_value_long_good_floor = 1
rescue_value_before500_ceiling = 15
rescue_value_before500_floor = 1
rescue_value_before500_fix_multi = 5
...
def simulator_game(record_data, total_round):  # 完整遊戲模擬
    ...
    for i in range(player):
        cnt_cum_range = observe_rtp_range_1  # 判定區間
        ...
        for j in range(total_round):
            ...
            if cnt_spin == cnt_cum_range_trigger:
                ...
                observe_rtp_range_2_temp = observe_rtp_range_2 if observe_rtp_range_2 < j else j
                rtp_2 = record_data_rescue_pay[i, j - observe_rtp_range_2_temp : j].sum() / observe_rtp_range_2_temp / threshold  # 往前抓"observe_rtp_range_2"場

                observe_rtp_range_3_temp = observe_rtp_range_3 if observe_rtp_range_3 < j else j
                rtp_3 = record_data_rescue_pay[i, j - observe_rtp_range_3_temp : j].sum() / observe_rtp_range_3_temp / threshold  # 往前抓"0~observe_rtp_range_3"場

                if cnt_spin < observe_rtp_range_2:
                    if (rtp_1 < return_rtp_threshold_1_before500) and (rtp_2 < return_rtp_threshold_2):
                        mode_trigger_rescue = 4
                else:
                    if (rtp_3 < return_rtp_threshold_3) and (rtp_1 < return_rtp_threshold_1_long_bad) and (rtp_2 < return_rtp_threshold_2):
                        mode_trigger_rescue = 1
                    if (rtp_3 >= return_rtp_threshold_3) and (rtp_1 < return_rtp_threshold_1_long_good) and (rtp_2 < return_rtp_threshold_2):
            ...
            if mode_trigger_rescue >= 1:
                ...
                # rescue
                if mode_trigger_rescue == 1:
                    pay = (return_percnetage_long_bad - rtp_1) * observe_rtp_range_1 * threshold
                    if pay >= rescue_value_long_bad_ceiling * threshold:  # 輸太多，只救到上限
                        pay = rescue_value_long_bad_ceiling * threshold
                    elif pay < rescue_value_long_bad_floor * threshold:  # 輸得太少，救不到
                        pay = row_data_pay[i, j]
                        mode_trigger_rescue = 0
                elif mode_trigger_rescue == 2:
                    pay = (return_percnetage_long_good - rtp_1) * observe_rtp_range_1 * threshold
                    if pay >= rescue_value_long_good_ceiling * threshold:  # 輸太多，只救到上限
                        pay = rescue_value_long_good_ceiling * threshold
                    elif pay < rescue_value_long_good_floor * threshold:  # 輸得太少，救不到
                        pay = row_data_pay[i, j]
                        mode_trigger_rescue = 0
                elif mode_trigger_rescue == 3:
                    pay = rescue_value_long_good_ceiling * threshold
                elif mode_trigger_rescue == 4:
                    pay = (return_percnetage_before500 - rtp_1) * observe_rtp_range_1 * threshold
                    # 救至固定RTP
                    if pay >= rescue_value_before500_ceiling * threshold:  # 輸太多，只救到上限
                        pay = rescue_value_before500_ceiling * threshold
                    elif pay < rescue_value_before500_floor * threshold:  # 輸得太少，救不到
                        pay = row_data_pay[i, j]
                        mode_trigger_rescue = 0
                    # # 救固定倍數
                    # pay = rescue_value_before500_fix_multi * threshold
            ...
            # update
            cnt_spin += 1
            if cnt_spin >= cnt_cum_range - 1:  # 更新判定區間/更新判定把數
                cnt_cum_range += trigger_range
                new_cnt_cum_range_trigger = cnt_cum_range - trigger_range + np.random.randint(0, trigger_range)
...
rtp_rescue_4 = (np.nansum(record_data_rescue_pay[record_data_rescue_mode == 4]) - np.nansum(row_data_pay[record_data_rescue_mode == 4])) / total_spin / threshold
...
print(f"  - rtp_rescue_4: {rtp_rescue_4*100:0.2f}% ({rtp_rescue_4/(rtp_rescue-rtp)*100:5.2f}%)")
...
print(f"  - 救援次數-救援4: {record_data_rescue_mode[record_data_rescue_mode==4].shape[0]}")
print(f"  - 平均救援次數:", f"{cnt_trigger_rescue/cnt_trigger_rescue_player:0.02f} 次/人")


print("\n* 救援強度")
multi_rescue_1 = np.nansum(record_data_rescue_pay[record_data_rescue_mode == 1]) / np.sum(record_data_rescue_mode == 1) / threshold
multi_rescue_2 = np.nansum(record_data_rescue_pay[record_data_rescue_mode == 2]) / np.sum(record_data_rescue_mode == 2) / threshold
multi_rescue_3 = np.nansum(record_data_rescue_pay[record_data_rescue_mode == 3]) / np.sum(record_data_rescue_mode == 3) / threshold
multi_rescue_4 = np.nansum(record_data_rescue_pay[record_data_rescue_mode == 4]) / np.sum(record_data_rescue_mode == 4) / threshold

print("  - 救援1:", f"{multi_rescue_1:0.02f}x")
print("  - 救援2:", f"{multi_rescue_2:0.02f}x")
# print("  - 救援3:", f"{multi_rescue_3:0.02f}x")
print("  - 救援4:", f"{multi_rescue_4:0.02f}x")
