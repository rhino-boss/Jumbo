...
| `game_id` | `overview.game_ID` | 直接 | `370`, `979`, `1006` | `1558` | 顯示與摘要 |
...
| `paylines` | `paylines.Line` | 直接 | `392`, `429`, `575`, `590`, `609` | `882`, `887`, `896`, `917`, `936` | 20 條線的列索引陣列 |
| `pay_table` | `pay_table.Symbol / 3 / 4 / 5 / Id` | 直接 | `350`, `432`, `569`, `578` | `874` | 依 symbol id 存 3/4/5 連線獎金 |
| `symbol_id` | `pay_table.Id` | 直接 | `595`, `664`, `759`, `769`, `772`, `793` | `1009`, `1402` | drop fill 時依索引取實際 symbol id |
| `symbol_str` | `pay_table.Symbol` | 直接 | `279`, `596`, `665`, `794` | `709`, `754`, `944`, `1031`, `1409` | id -> symbol code |
| `base_symbol_of` | 由 `pay_table.Symbol` 的 base / gold 對應關係產生 | 衍生 | `276`, `502`, `773` | `807`, `1010`, `1408` | 例如 `G1 -> M1` |
| `is_gold_symbol` | 由 symbol code 是否為 gold 版產生 | 衍生 | `277`, `504`, `775` | `809`, `1012` | `G1~GJ` 為 1 |
| `is_score_symbol` | 由 `pay_table` 可否得分產生 | 衍生 | `278`, `431`, `567`, `577` | `873` | Wild/Scatter 為 0，一般符號為 1 |
| `symbols_score` | 由 `pay_table` 可得分符號 id 清單產生 | 衍生 | `430`, `576`, `619` | `892` | 盤面評獎迴圈使用 |
| `value_multiplier_range` | `value.Multiplier_Range` | 直接 | `341`, `381`, `447` | `730` | 倍數值清單 |
| `weight_table_bg` | `weight.Table_BG` | 直接 | `272`, `384`, `465` | `776` | BG 選 table 權重 |
| `weight_table_fg` | `weight.Table_FG` | 直接 | `273`, `385`, `469` | `780` | FG 選 table 權重 |
| `weight_table_bf` | `weight.Table_BF` | 直接 | `274`, `386`, `473` | `784` | BF 選 table 權重 |
| `weight_cum_table_bg` | 由 `weight.Table_BG` 累加 | 衍生 | `387`, `463` | `774` | BG 選 table 用 cumulative weight |
| `weight_cum_table_fg` | 由 `weight.Table_FG` 累加 | 衍生 | `388`, `467` | `778` | FG 選 table 用 cumulative weight |
| `weight_cum_table_bf` | 由 `weight.Table_BF` 累加 | 衍生 | `389`, `471` | `782` | BF 選 table 用 cumulative weight |
| `fg_table_rule` | 推定來自 generator / 規則設定 | 靜態/外部 | `390` | 未使用 | 目前 runtime 沒拿它做 FG table 選擇 |
| `weight_multiple_special` | `weight.Multiple_Selection_Weight_Special_*` | 直接 | `338` | 未直接取 raw weight | special gold 倍數 raw weight |
| `weight_multiple_r3_before` | `weight.Multiple_Selection_Weight_R3_Before_*` | 直接 | `336` | 未直接取 raw weight | R3 初始 gold 倍數 raw weight |
| `weight_multiple_r3_after` | `weight.Multiple_Selection_Weight_R3_After_*` | 直接 | `337` | 未直接取 raw weight | R3 drop 後 gold 倍數 raw weight |
| `weight_multiple_before` | `weight.Multiple_Selection_Weight_Before_*` | 直接 | `334` | 未直接取 raw weight | 非 R3 初始 gold 倍數 raw weight |
| `weight_multiple_after` | `weight.Multiple_Selection_Weight_After_*` | 直接 | `335` | 未直接取 raw weight | 非 R3 drop 後 gold 倍數 raw weight |
...
| `weight_special_pool` | `weight.Used_Special_Pool_Weight_*` | 直接 | `539` | `839` | Base 特殊金框池觸發率 |
| `arr_reels` | `BG_strip* / FG_strip* / BF_strip.Symbol_ID_R1~R5` | 直接 | `280`, `501` | `803`, `806` | 各 table reel strip 符號表 |
| `arr_reels_weight` | `BG_strip* / FG_strip* / BF_strip.Strip_Weight_R1~R5` | 直接 | `281` | 未使用 | raw reel weight，HTML/Simulator 都實際用 cum 版 |
| `arr_reels_weight_cum` | 由 `arr_reels_weight` 累加 | 衍生 | `489` | `803` | 停輪抽樣用 cumulative weight |
| `drop_weight_a` | `BG_strip* / FG_strip* / BF_strip.Drop_Weight_A_R1~R5` | 直接 | 未使用 | `1007` | HTML 只被關鍵字命中；runtime 實際用 cum 版 |
| `drop_weight_b` | `BG_strip* / FG_strip* / BF_strip.Drop_Weight_B_R1~R5` | 直接 | 未使用 | `1007` | 同上 |
...
| --- | --- | --- |
| `game_ID` | `game_id` | 直接對應 |
| `game_version` | 無 | 目前沒進 `config.js` |
| `game_RTP` | 無 | 目前沒進 `config.js`；RTP 是模擬結果，不是 runtime 輸入 |

### B2. `pay_table`

| xlsx 欄位 | config.js 欄位 | 備註 |
| --- | --- | --- |
| `Symbol` | `symbol_str` | id -> code 對照 |
| `Id` | `symbol_id` | symbol id 清單 |
| `3`, `4`, `5` | `pay_table` | 每個 symbol 的 3/4/5 連線獎金 |
| `Symbol + Id` | `symbols_score` | 過濾可得分的 symbol id |
| `Symbol` | `base_symbol_of` | 由 `G*` 對應回 base symbol |
| `Symbol` | `is_gold_symbol` | `G*` 類型標記 |
| `3/4/5` 是否全為 0 | `is_score_symbol` | 無獎金的 symbol 視為不可計分 |

### B3. `value`

| xlsx 欄位 | config.js 欄位 | 備註 |
| --- | --- | --- |
| `Multiplier_Range` | `value_multiplier_range` | 倍數值清單 |

### B4. `paylines`

| xlsx 欄位 | config.js 欄位 | 備註 |
| --- | --- | --- |
| `Line` | `paylines` | 例如 `11111` 轉為 `[1,1,1,1,1]` |

### B5. `BG_strip`, `BG_strip (2)`, `BG_strip (3)`, `FG_strip`, `FG_strip (2)`, `FG_strip (3)`, `BF_strip`
...
| --- | --- | --- |
| `Symbol_ID_R1~R5` | `arr_reels` | 各 table 各 reel 的符號 strip |
| `Strip_Weight_R1~R5` | `arr_reels_weight` | raw strip weight |
| `Strip_Weight_R1~R5` | `arr_reels_weight_cum` | 對 raw weight 做 cumulative sum |
| `Drop_Weight_A_R1~R5` | `drop_weight_a` | raw drop weight A |
| `Drop_Weight_A_R1~R5` | `drop_weight_a_cum` | cumulative 版 |
| `Drop_Weight_B_R1~R5` | `drop_weight_b` | raw drop weight B |
| `Drop_Weight_B_R1~R5` | `drop_weight_b_cum` | cumulative 版 |
...
| xlsx 欄位群 | config.js 欄位 | 備註 |
| --- | --- | --- |
| `Table_BG` | `weight_table_bg` | BG 選 table raw weight |
| `Table_FG` | `weight_table_fg` | FG 選 table raw weight |
| `Table_BF` | `weight_table_bf` | BF 選 table raw weight |
| `Table_BG/FG/BF` | `weight_cum_table_bg/fg/bf` | cumulative 版 |
| `Multiple_Selection_Weight_Special_*` | `weight_multiple_special` | special gold 倍數 raw weight |
| `Multiple_Selection_Weight_Special_*` | `weight_cum_multiple_special` | cumulative 版 |
| `Multiple_Selection_Weight_R3_Before_*` | `weight_multiple_r3_before` | R3 初始 gold 倍數 raw weight |
| `Multiple_Selection_Weight_R3_Before_*` | `weight_cum_multiple_r3_before` | cumulative 版 |
| `Multiple_Selection_Weight_R3_After_*` | `weight_multiple_r3_after` | R3 cascade 後 gold 倍數 raw weight |
| `Multiple_Selection_Weight_R3_After_*` | `weight_cum_multiple_r3_after` | cumulative 版 |
| `Multiple_Selection_Weight_Before_*` | `weight_multiple_before` | 非 R3 初始 gold 倍數 raw weight |
| `Multiple_Selection_Weight_Before_*` | `weight_cum_multiple_before` | cumulative 版 |
| `Multiple_Selection_Weight_After_*` | `weight_multiple_after` | 非 R3 cascade 後 gold 倍數 raw weight |
| `Multiple_Selection_Weight_After_*` | `weight_cum_multiple_after` | cumulative 版 |
| `Used_Special_Pool_Weight_*` | `weight_special_pool` | 特殊池觸發機率表 |
| `Eliminate_Table_Weight_BG/FG` | 無明確對應 | 目前這兩欄沒有直接出現在 `config.js` 欄位名中 |
...
下列欄位在目前 `H026_math_data.xlsx` 的 sheet / header 中沒有直接看到明確欄位，應視為 generator 或外部規則輸入：
