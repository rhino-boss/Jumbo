...
- `RUN_ALL_COMBINATIONS`：H026 式批次執行開關；預設 `False`，避免一般啟動時意外跑完整批次。
- `BATCH_RUNS`：批次組合，可分別指定 config、模式、局數、玩家 Profile 與卡片系統開關。
- `THREADS`：平行執行緒數；預設最多 8，避免 IDE 直接執行時占滿 CPU／記憶體。
...
單次執行與 H026 一樣會固定輸出精簡摘要；`SHOW_CONSOLE_SUMMARY`／`SHOW_CONSOLE_DETAIL` 只控制完整表格是否展開。
...
先在 `Simulator.py` 的 `BATCH_RUNS` 取消註解或加入組合，再執行：

```powershell
$env:H015_RUN_ALL_COMBINATIONS='true'
.\.venv\Scripts\python.exe '.\Project_AI\Slots\H015_賞金列車\Simulator.py'
```

每個組合會由獨立子程序執行，避免不同模式的 Numba 全域資料互相污染。子程序的啟動、Numba 編譯與結果會即時顯示，不會等整批結束才一次出現。
...
- 報表會列出 retry 總數、達上限次數與 BG Range／BG Free Game／FG 失敗分類；啟用卡片系統時檔名會加入 RTP、Profile 與 `_card` 標記。
